...
async function main() {
  ...
  console.log(
    `[dev-desktop] Starting @sediment/server (tsx watch) on :${SERVER_PORT} …`,
  );
  ...
  //    port pair the rest of the dev tooling expects.
  console.log(`[dev-desktop] Starting @sediment/web (Vite) on :${VITE_PORT} …`);
  ...
  try {
    await Promise.all([
      waitForPort(HOST, SERVER_PORT, READY_TIMEOUT_MS),
      waitForPort(HOST, VITE_PORT, READY_TIMEOUT_MS),
    ]);
  } catch (err) {
  ...
  console.log(
    `[dev-desktop] server up at http://${HOST}:${SERVER_PORT}, vite up at http://${HOST}:${VITE_PORT}`,
  );
  ...
  const electronEnv = {
    ...process.env,
    WEB_DEV_SERVER_URL: `http://${HOST}:${VITE_PORT}`,
    EXTERNAL_SERVER_URL: `http://${HOST}:${SERVER_PORT}`,
  };

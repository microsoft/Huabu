...
export interface NodeStyle {
  ...
  /**
   * Authored font size (px) for Text and Question. Relative to the type's
   * default, it also scales content padding and Question's full card chrome.
   * Corner, Hug Frame and selection scaling multiply this by the geometry
   * ratio; width-only reflow and content edits preserve it. An absent value
   * uses the node type's default. Note/document typography is independent.
   */
  fontSize?: number;
}
...
export interface AutoHeightHint {
  /**
   * Note content height in canvas CSS pixels at its measured inner width,
   * including host padding but excluding shell chrome. The conversion to the number
   ...
   * pipeline version, content revision, and inner width. Only a real
...
export interface VideoNodeData extends BaseNodeData {
  type: 'video';
  src: string;
  /** Space-local JPEG artifact key generated by preprocessing. */
  coverUrl?: string;
  /** Exact source this cover represents; ignore the cover when it differs from src. */

...
/**
 * Trusted catalogue of ACP-capable agents the daemon knows how to
 * launch. Kept separate from the probing logic in
 * {@link ./detect} so adding / editing an agent is a pure data
 ...
 *
 * This is intentionally a daemon-local module, with no remote catalogue
 * or caller-supplied commands. IDs also select daemon-owned workspaces.
 */

import type { HarnessCatalogueEntry } from '@agentlet/protocol';

/** One known external agent's detection + launch recipe. */
export interface KnownCli extends HarnessCatalogueEntry {
  /**
...
/**
 * Canonical catalogue of ACP-capable CLIs. Stable order is retained in
 * discovery responses for hosts presenting the catalogue.
 */

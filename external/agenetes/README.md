...
- **`@agenetes/agent-team`** — the host-agnostic Agent Team control plane: owns collection-root and discovered-member identities, root-to-member provenance reconciliation, missing-member retention, and schema-versioned atomic registry persistence below a host-provided local storage directory.

...
Every turn is appended to a per-thread log so a conversation survives restarts and idle-out. Consumers can replay history and subscribe to the live tail. Orthogonal to transport. _(K8s: etcd + the watch API.)_

每一轮都追加进 per-thread 的日志，使会话能挺过重启与 idle 挂起。消费者可回放历史，也可订阅实时尾流。与传输正交。_(对应 K8s：etcd + watch API。)_

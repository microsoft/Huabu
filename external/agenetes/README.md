...
A driver is its metadata (`AgentDriverInfo` — the `AgentCapabilities` every one of its handles advertises) plus a single factory method. The dispatch `kind` is supplied by the instance registry (`register(kind, driver)`), not carried by the driver object itself:

一个 driver = 它的元数据（`AgentDriverInfo`——它每个 handle 都声明的 `AgentCapabilities`）+ 一个工厂方法。分发键 `kind` 由 instance registry（`register(kind, driver)`）提供，并不由 driver 对象自己携带：

...
`AgentTurn`（在 `@agenetes/protocol`）是 *delta* 的 `AgentStreamEvent` 的 *折叠* 孪生，共用同一套事件词汇——`{ request, transcript }`，transcript 是 driver-agnostic 的折叠形态（**不是**任何单一宿主的消息形状；某宿主自己的消息数组降级为它为自身 context 组装派生的 L1 投影，正如 `run` 的 `TResult`，I9.4）。因为 L1 不能 append，**fork**（从另一 thread 历史播种新 thread）不再能表达为 L1 的 turn 拷贝；它是独立的 thread 生命周期关注点，作为 unsupported-legacy-feature 债务追踪，直到它挣得自己的 L2 播种原语——刻意**不**属于这条只读表面。

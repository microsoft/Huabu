...
The Profile registry supports `launch: { kind: 'acp-command', command }` and `launch: { kind: 'acp-harness', harnessId, options?: { autoApprove?: boolean } }`. `createProfile()` selects these via `launchKind`. `patchProfile()` permits working-directory and launch-option/command edits, but switching launch kind, harness ID, or agentlet ID requires a new Profile. Every legacy command maps to the Custom wrapper without inference from command text or metadata; no records are rewritten. `@agenetes/protocol` exports the strict `agentProfileLaunchSchema`, `acpHarnessLaunchSchema`, `harnessLaunchOptionsSchema`, and `harnessLaunchPlanSchema` contracts. Unknown structured options are rejected, and editable `metadata.cliId` never controls execution.

New Profiles start with `revision: 0` and `executionRevision: 0`. Both fields remain optional in persisted data: absence means zero and loading does not migrate old records. A changed patch increments `revision`; changing cwd or launch also increments `executionRevision`. No-op patches neither persist nor notify. Optional `PatchAgentProfileInput.expectedRevision` rejects stale edits with `profile_conflict`, including stale no-op requests. Alias, metadata, and opaque custom-data edits do not change the execution revision. Snapshots remain independent deep clones and carry optional `executionRevision` for host workload tagging; existing snapshots and bindings are never updated by later edits or deletion.

The ACP driver accepts optional `profileExecutionRevision` on its spec, separately from the process recipe. Profile lowering propagates snapshot execution revisions, and session opening and recovery forward that frozen value to `AcpProfileCachePort.readCommands(profileId, profileExecutionRevision?)`. Hosts compare this identity with their current Profile revision before returning warm-start commands; absent legacy values mean zero. The optional argument preserves compatibility with existing single-argument cache adapters.
...
`AgentletGateway.buildHarnessLaunch(agentletId, { launch: AgentProfileLaunch })` targets the explicit machine and requires `capabilities.harnessLaunchPreview.version === 1`, independently from discovery and spawn support. It validates the request and daemon response and returns `{ kind: 'exec', executable, argv, env }` or `{ kind: 'shell', command }`. This read-only daemon compilation never spawns or prepares workspaces; hosts must not manufacture argv. Old daemons fail with `harness_launch_preview_unsupported`, malformed replies with `invalid_harness_launch_preview_response`, and disconnected targets with `agentlet_disconnected`. Discovery's optional `launchPreviewVersion: 1` and `capabilities.customLaunchCommand` are validated without requiring them from old daemons. The `custom` discovery descriptor is manually selectable, never automatically provisioned; only its custom-command capability is supported.

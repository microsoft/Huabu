...
The agreed principle is that the instance's surface stays deliberately narrow and identity-addressed: the host names a session by its `threadId` (I4.2) rather than juggling handles, and whatever host→agent control the instance exposes ultimately delegates to the handle's `control(msg)` (I8.2, the shipped contract) — the closed vocabulary, capability gating, and `ControlAck` do not change by being reached through the instance. Equally agreed is what the surface **excludes**: concerns that depend on host data or host UX stay **host responsibilities**, not core surface — get-or-create-with-spec (spawn orchestration), profile/schema caching, and cold-start UX (painting a toolbar before the agent's authoritative state arrives, a host cache fed from the metadata the instance surfaces).
已达成的原则是：实例的表面刻意保持**狭窄**且**按身份寻址**——宿主用 `threadId`（I4.2）指名一个 session，而不是自己摆弄一堆 handle；实例对外暴露的任何 host→agent 控制，最终都委托给 handle 的 `control(msg)`（I8.2，已落地的契约）——封闭词汇、能力门控、`ControlAck` 不因经由实例而改变。同样已达成的是这个表面**排除**什么：凡依赖宿主数据或宿主 UX 的关注点都留作**宿主职责**、不进核心表面——带 spec 的 get-or-create（spawn 编排）、profile/schema 缓存、以及冷启动 UX（在 agent 的权威状态到达前先画好工具栏，一个由实例surface出的元数据喂养的宿主缓存）。

> **Not yet consensus — the exact instance API is in design (step D). / 尚未成为共识——确切的实例 API 仍在设计中（step D）。** The concrete method set and signatures (e.g. a `run(spec, ctx)` turn-driver, an `ensure(spec)` no-turn open, `control(threadId, msg)`, `get` / `close` / `getMeta`, and a `notifications()` push stream) are a **working draft** in the refactor plan ([`docs/proposals/layered-architecture.md`](../../docs/proposals/layered-architecture.md) §7 M5), **not** a settled invariant, and must not be cited as one until confirmed.
> 具体的方法集合与签名（例如驱动一轮的 `run(spec, ctx)`、开一个不含轮次的 `ensure(spec)`、`control(threadId, msg)`、`get` / `close` / `getMeta`，以及一条 `notifications()` 推送流）目前只是重构计划（[`docs/proposals/layered-architecture.md`](../../docs/proposals/layered-architecture.md) §7 M5）里的**工作草案**，**不是**已确定的不变量；在确认之前，不得作为共识被引用。

...
A single **run/turn** — `run(request, render, ctx)` — is the unit both workload kinds share, since a run *is* "submit this turn's input, then stream its output" (M2's separate `submit` + `events()` merged into one call). A **Job** *is* exactly one run then terminal (its handle's life == the run; `close()` a no-op); a **Deployment** *has-a* run-producer: a long-lived session hosting many runs plus `control` / notifications / liveness. Content updates (message / tool / plan / thought) flow as the run's yielded `AgentStreamEvent`s — the **data plane**. Affordance/meta traffic is the **control plane**, and it is split by direction: host→agent operations travel as `ControlMsg` via `control` (`set_mode`, `cancel`, `answer_permission`, …), while agent→host affordance updates (`available_commands_update`, `current_mode_update`, `permission_request`) ride the same `AgentStreamEvent` stream as notifications — logically distinct from content even when both share one physical stream. Slash follows ACP exactly: discover via an `available_commands_update` notification, *invoke* by putting the command text into an ordinary data-plane prompt — there is no `runCommand`.
一次**run/轮次**——`run(request, render, ctx)`——是两种工作负载种类共享的单元，因为一次 run *就是*"提交本轮输入、再流式输出其结果"（M2 中分开的 `submit` + `events()` 已合并为一次调用）。**Job** *就是*恰好一次 run 然后终止（其 handle 的生命 == 那次 run；`close()` 为空操作）；**Deployment** *拥有*一个 run 生产者：一个长期存活、承载多次 run 的 session，外加 `control` / 通知 / 存活性。内容更新（message / tool / plan / thought）作为 run 产出的 `AgentStreamEvent` 流动——即**数据面**。能力/元信息流量是**控制面**，且按方向拆分：host→agent 的操作作为 `ControlMsg` 经 `control` 发送（`set_mode`、`cancel`、`answer_permission`……），而 agent→host 的能力更新（`available_commands_update`、`current_mode_update`、`permission_request`）作为通知走在同一条 `AgentStreamEvent` 流上——即便二者共用同一条物理流，在逻辑上仍与内容不同。斜杠命令完全遵循 ACP：经 `available_commands_update` 通知*发现*，通过把命令文本放进普通的数据面 prompt 来*执行*——没有 `runCommand`。
...
The core L2 surface stays deliberately narrow — session lookup / lifecycle (`get` / `close`), the host→agent `control(threadId, msg): Promise<ControlAck>` operation, and a `notifications()` stream of metadata changes. `control` is **host→agent only, not duplex**: `ControlMsg` is a closed, Agenetes-owned vocabulary of five operations — `cancel`, `set_mode`, `set_model`, `set_config_option`, `answer_permission` — each capability-gated (a Job honours only `cancel`; a Deployment the full set) and resolving to a minimal `ControlAck` (`{ ok: true }`, or a failure with a `code` like `unsupported`), never a stream. The agent→host direction is *not* `control`: it rides the run's `AgentStreamEvent`s (e.g. `permission_request`, `available_commands_update`, `current_mode_update`) — `answer_permission` is precisely the host *replying* to a `permission_request`, correlated by `requestId`. Concerns that depend on host data or host UX stay **host responsibilities**, not core surface: get-or-create-with-spec (spawn orchestration), profile/schema caching, and cold-start UX (painting a toolbar before the agent's authoritative state arrives — a host cache fed by subscribing to `notifications()`).
核心 L2 表面刻意保持狭窄——session 查找 / 生命周期（`get` / `close`）、host→agent 的 `control(threadId, msg): Promise<ControlAck>` 操作，以及一条元数据变化的 `notifications()` 流。`control` 是 **host→agent 单向的，不是双工**：`ControlMsg` 是一个由 Agenetes 拥有的封闭词汇，共五个操作——`cancel`、`set_mode`、`set_model`、`set_config_option`、`answer_permission`——每个都受能力门控（Job 只认 `cancel`；Deployment 认全套），并解析为一个极简 `ControlAck`（`{ ok: true }`，或一个带 `code`（如 `unsupported`）的失败），绝非一条流。agent→host 方向*不是* `control`：它走 run 的 `AgentStreamEvent`（例如 `permission_request`、`available_commands_update`、`current_mode_update`）——`answer_permission` 恰恰是 host 对某个 `permission_request` 的*回复*，按 `requestId` 关联。凡依赖宿主数据或宿主 UX 的关注点，都留作**宿主职责**、不进核心表面：带 spec 的 get-or-create（spawn 编排）、profile/schema 缓存、以及冷启动 UX（在 agent 的权威状态到达前先画好工具栏——一个通过订阅 `notifications()` 喂养的宿主缓存）。

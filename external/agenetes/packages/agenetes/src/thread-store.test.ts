...
import type { AgentMetadata, Namespace } from '@agenetes/protocol';
...
describe('FileThreadStore (M5.5/A2 durable backing)', () => {
  it('round-trips spec + state (storageRoot, sessionId, metadata)', () => {
    const store = new FileThreadStore();
    const namespace = ns('canvas-1');
    const record: ThreadRecord<Spec> = {
      spec: spec('t1', 'hello'),
      state: new AgentPersistentState('/root/canvas-1', 'sess-abc', meta),
    };
    ...
    expect(reread!.state.storageRoot).toBe('/root/canvas-1');
  ...
  it('upsert replaces the whole record; delete removes it', () => {
    ...
    store.upsert(namespace, 't1', {
      spec: spec('t1'),
      state: new AgentPersistentState(undefined, 'first'),
    });
    store.upsert(namespace, 't1', {
      spec: spec('t1'),
      state: new AgentPersistentState(undefined, 'second'),
    });
  ...
  it('lists only the current namespace and isolates across namespaces', () => {
    ...
    store.upsert(a, 't1', {
      spec: spec('t1'),
      state: new AgentPersistentState(undefined),
    });
    store.upsert(b, 't2', {
      spec: spec('t2'),
      state: new AgentPersistentState(undefined),

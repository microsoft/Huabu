...
> Annotation node 的二次设计：把它从「画一笔 → 等 3s → AI 自动消费的临时手势」重塑成「可持久化、可分组、可显式触发 AI 解读的草图节点」。
...
| 不破坏现有数据               | `type: 'annotation'` 暂不改名；现有 canvas 文件零迁移可直接打开。                                      |
...
| `annotation`（即 sketch）                                     | **全部数据**          | ❌ 无                         | `points: number[][]` / `initialSize` / `executed` 等 inline 进 `state.nodes[i].data` |
...
- **节点结构**：`AnnotationNodeData`（[node.ts:323-340](../packages/shared/src/types/canvas/node.ts#L323-L340)），含 `points` / `initialSize` / `strokeColor` / `executed`。
- **绘制工具**：`AnnotationOverlay`（[AnnotationOverlay.tsx](../apps/web/src/components/Nodes/annotation/AnnotationOverlay.tsx)）—— pointer 事件捕获 + 实时 SVG 预览 + 创建 annotation node。
- **空间聚类**：`clusterAnnotations()`（[clustering.ts](../apps/web/src/handler/annotation/clustering.ts)）—— Union-Find，bbox 边距 ≤ 200px。
- **上下文抽取**：`extractAnnotationContext()`（[context.ts](../apps/web/src/handler/annotation/context.ts)）—— 收集 cluster bbox 周围节点 / 边的 wire ref。
- **截图**：`captureCanvasScreenshot()`（[screenshot.ts](../apps/web/src/handler/canvasCommand/utils/screenshot.ts)）。
- **服务端 LLM 管线**：`recognizeAnnotationCommands()`（[annotation.service.ts](../apps/server/src/modules/agent/annotation.service.ts)）—— 截图 + 上下文 → vision LLM → canvas commands。
- **状态机 / 浮窗**：`AnnotationProcessingOverlay`（[AnnotationProcessingOverlay.tsx](../apps/web/src/components/Nodes/annotation/AnnotationProcessingOverlay.tsx)）—— preparing / pending / running / done 四态 + Accept / Revert 按钮。
...
2. **识别成功后笔迹被打半透明灰**（`executed: true` 触发 `opacity: 0.25`，[AnnotationNode.tsx:31-33](../apps/web/src/components/Nodes/annotation/AnnotationNode.tsx#L31-L33)），暗示「这个节点已死」，与「sketch 是普通节点」的诉求冲突。
...
| 改动                  | 文件 / 模块                                                                               | 说明                                                                                                                  |
| --------------------- | ----------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| 删除 3s idle timer    | [intentStore.ts](../apps/web/src/store/intentStore.ts) `onAnnotationCreated`              | 不再 `setTimeout(triggerAnnotationRecognition, 3000)`；只更新 `pendingAnnotationIds` 用于下次显式触发的入参。         |
...
| 移除自动 timer 副作用 | [intentStore.ts](../apps/web/src/store/intentStore.ts)                                    | `cancelAnnotationRecognition` 仍然能 abort 进行中的批次，并清空 overlay。                                             |
| Sketch 不再 dim       | [AnnotationNode.tsx](../apps/web/src/components/Nodes/annotation/AnnotationNode.tsx)      | 移除 `executed` → `opacity: 0.25`。`executed` 字段保留（Accept/Revert 状态机仍在用）。                                |
| 多选 toolbar 加按钮   | [MultiSelectToolbar.tsx](../apps/web/src/components/Panels/Canvas/MultiSelectToolbar.tsx) | 选中 ≥2 节点且**全部** `type === 'annotation'` 时显示 `✨ Apply Sketch` 按钮，onClick 调 `requestSketchRecognition`。 |
| 单选 toolbar 加按钮   | [AnnotationNode.tsx](../apps/web/src/components/Nodes/annotation/AnnotationNode.tsx)      | 把 `toolbar={<ApplySketchButton />}` 传进 `NodeWrapper`，onClick 同上但只传 `[id]`。                                  |

**未改动**：

- 服务端 `annotation.service.ts`、`intent.route.ts`、prompt 全部不动。
- 共享类型 `AnnotationNodeData`、`AnnotationCluster`、`AnnotationStroke` 不动。
...
- 重命名 `annotation` → `sketch`（若 Phase 2 验证后觉得有必要再统一收）。
...
| 改动                               | 文件 / 模块                                                                  | 说明                                                                                                                                                                     |
| ---------------------------------- | ---------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `AnnotationStroke.createdAt`       | [intent.ts](../packages/shared/src/types/agent/intent.ts) `AnnotationStroke` | 时间窗聚类需要。                                                                                                                                                         |
...
| `AnnotationNodeData.userExtracted` | [node.ts](../packages/shared/src/types/canvas/node.ts) `AnnotationNodeData`  | sticky 标记：用户手动从 group 拖出后置位，再也不会被自动吸回。                                                                                                           |
| 自动 group 调度器                  | 新文件 `apps/web/src/handler/annotation/autoGroupSketches.ts`                | sketch settle 800ms 后跑 `clusterAnnotations`（带时间窗 ≤ 10s + 跳过 `userExtracted`），命中 ≥ 2 笔 → 创建 `sketch-group` frame 父节点，把 sketches 设为它的子节点。     |
...
用户画 N 笔                           用户圈选 sketch
   │                                      │
   ▼                                      ▼
AnnotationOverlay.handlePointerUp     React Flow 多选/单选
   │                                      │
   ▼                                      ▼
addNode({type: 'annotation', ...})    selected: true
   │                                      │
   ▼                                      ▼
intentStore.onAnnotationCreated(id)   MultiSelectToolbar / NodeToolbar
   │                                      │
   │ (只更新 pendingAnnotationIds        │ ✨ Apply Sketch onClick
   │  用于聚类预览，不再启 timer)        │
   │                                      ▼
   ▼                                  intentStore.requestSketchRecognition(ids)
（什么都不会自动发生）                     │
                                          ├── Stage 1: clusterAnnotations(strokes)
                                          ├── Stage 2: extractAnnotationContext(cluster, ...)
...
| 历史 canvas 有 dim 灰笔迹     | 老 canvas 中的 `executed: true` annotation 之前是半透明的。      | Phase 1 移除 dim 渲染后，老笔迹会自动恢复正常显色，无需 migration（数据字段不动）。 |
...
- **重命名 `annotation` → `sketch`**：50+ 文件机械改动 + 持久化数据兼容（旧 canvas.json 里 `type: 'annotation'`）。验证完 Phase 1/2 体验后再决定是否做。
...
| 绘制工具            | [apps/web/src/components/Nodes/annotation/AnnotationOverlay.tsx](../apps/web/src/components/Nodes/annotation/AnnotationOverlay.tsx) |
| Sketch 节点渲染     | [apps/web/src/components/Nodes/annotation/AnnotationNode.tsx](../apps/web/src/components/Nodes/annotation/AnnotationNode.tsx)       |
| 客户端管线          | [apps/web/src/store/intentStore.ts](../apps/web/src/store/intentStore.ts)                                                           |
| 聚类                | [apps/web/src/handler/annotation/clustering.ts](../apps/web/src/handler/annotation/clustering.ts)                                   |
| 上下文              | [apps/web/src/handler/annotation/context.ts](../apps/web/src/handler/annotation/context.ts)                                         |
| 服务端识别          | [apps/server/src/modules/agent/annotation.service.ts](../apps/server/src/modules/agent/annotation.service.ts)                       |

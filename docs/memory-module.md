...
**单文件约束**：long-term 和 short-term 都是单 markdown 文件，硬上限 4 KB body / 80 行。超限触发自压缩。Skill 没有体积上限，但新增门槛极高（见 §3.3）。
...
- `memory_longterm_write` — 合并到 `setting/.huabu.md`，去重 + 行数压缩。
- `memory_shortterm_write` — 整体覆盖 `<canvas>/.memory/canvas.md`，超长触发同 LLM 自压缩。
...
- 三个 memory write tool 都用现有 `safeResolve` 同款 sandbox 思路，但根目录换成 workspace（long-term / skills）或 `<canvas>/.memory/`（short-term）。新建 `memorySandbox` 模块，禁止 `..` 逃逸。
...
export function longTermMemoryPath(): string; // <workspace>/setting/.huabu.md
...
| 现有                                                                                                 | 变更                                                                 |
| ---------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------- |
| [`CanvasStore.readPreferences/writePreferences`](../apps/server/src/modules/storage/canvas-store.ts) | 删除（被 short-term writer 取代）                                    |

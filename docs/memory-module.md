...
工具描述里只写 mechanics（参数 / cap / 校验），**策略放在 SKILL.md 里**：
...
│   │       ├── writers.ts                 # overwrite + replace_string 两个原语
│   │       ├── sandbox.ts                 # 双根路径校验
...
│   └── agent/tools/
│       ├── definitions.ts                 # fs_write 工具定义
│       └── handlers/
│           ├── fs-write.ts                # path 路由 + mode 分派 → writers
│           └── fs-read.ts                 # read("memory/*.md") 分支
└── prompt/
    ├── agents/
    │   ├── memory/AGENT.md                # 后台 curator 的 system prompt
    │   ├── ask/AGENT.md                   # 含 fs_write + memory section
    │   └── operate/AGENT.md               # 同上
    └── skills/

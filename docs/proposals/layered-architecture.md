...
**M3 — Home control + the notifications upstream on the long-lived Deployment handle.** *(What remains of the original "one control channel" once M2 / M2.6 land.)*
- *Goal:* now that the `control(msg)` + `capabilities` vocabulary (shipped in M2) has a correct home (the long-lived handle, M2.6), grow the agent→host out-of-turn **upstream** (`notifications()`) as the dual of `control`, and split the ACP-shaped REST side-band cleanly across the seam: L2 stays pure `get` / `control` / `notifications()` (no spawn, no cache); L1 owns the profile-schema cache and the get-or-create-with-spec orchestration ([§3.6.2](#362-the-upward-interface--agenthandle-the-duplex-control-channel-and-capability-negotiation)).
- *Key correction landed here.* The per-profile schema cache ([`profile-schema-cache.ts`](../../apps/server/src/modules/agent/acp/profile-schema-cache.ts)) that seeds the toolbar dropdowns on a cold thread (browse / pre-select **before** paying the spawn cost) is an **L1 UX concern** — its *data* originates in L2 (agent `session/update` pushes) but the *caching policy*, cold-start seeding, and "spawn only on real change" orchestration are L1's. So M3 re-homes the cache to L1 and changes how it is fed (drain the handle's `notifications()`), rather than the L2 ACP layer directly mirroring an entry listener into a cross-thread cache.
- *L2 deliverables (subtree + ACP driver):* define the agent→host notification union in [`@agenetes/protocol`](../../external/agenetes/packages/protocol) (`available_commands_update` / `current_mode_update` / `config_option_update` / `session_info_update` / `usage_update`, or one generic envelope) — the upstream dual of `control`; add a `notifications(): AsyncGenerator<Notification>` facet to `AgentHandle` (a new subtree contract, like `run` / `close` in M2.6); the ACP handle implements it by surfacing the out-of-turn `session/update` meta pushes currently handled by `handleSessionMetaUpdate`. `get(threadId)` stays pure (M2.6) and `control()` stays homed (M2.6) — **L2 neither spawns nor caches.**
- *L1 deliverables (host):* claim `profile-schema-cache` as L1 and feed it by draining `handle.notifications()` (move the `mirrorEntryToProfileCache` step out of the L2 listener); fold the four **write** control routes — `POST /permission` → `get(threadId)?.control(answer_permission)` (assumes a live session; missing = precondition failure), and `POST /{mode,model,config}` → L1 orchestration: `create(threadId, spec)` (L1 holds the profile binding = spec; get-or-create the live session) + `handle.control(set_mode / …)`. `GET /{commands,cached-meta,meta}` stay L1 **reads** of L1's cache (a read, not a `control` op). Reverse permission stays a duplex correlated by `requestId` — already the shape (`answer_permission` control ⟂ `permission_request` event), so it is a reframing, not new work.
- *Sub-decision (flagged, resolve during M3):* live-entry meta (`currentModeId` / `availableModes` — a running session's own state) plausibly stays tracked in L2; the cross-thread, on-disk, cold-start snapshot is L1's cache. The listener then splits: L2 updates the live entry meta **and** emits on `notifications()`; L1 subscribes and mirrors into the profile cache.
- *Scope boundary — poll→push is M6, not M3.* M3 only changes **how the cache is fed** (via `notifications()`) and **who owns it** (L1); the cache itself and the browser `GET`-poll for out-of-turn updates stay, because without a standing browser channel the browser cannot receive between-turn pushes. Replacing the L1 `GET`-poll with a real push channel is M6. The browser hop stays the SSE↓ + POST↑ bridge — L1's own transport, **not** part of the L1↔L2 seam.
- *Depends on:* M2.6.
- *Acceptance:* `@agenetes/protocol` names the notification union and `AgentHandle` exposes `notifications()`; the ACP handle emits out-of-turn meta on it; L1 owns `profile-schema-cache` fed by draining `notifications()`; the four write routes go through `handle.control(...)` (permission via `get(threadId)`, mode/model/config via L1 `create(spec)` + `control`) and carry no spec into L2; `GET` reads stay L1 cache reads; permission / mode / slash flows work end-to-end; L2 neither spawns nor caches; UI unchanged or minimally adapted; full `apps/server` suite + `pnpm -r typecheck` green.

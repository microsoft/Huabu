...
- *What must be component-ized to move the glue (the crux of option B).* Three files relocate into `@agenetes/agentlet-host`: `daemon-auth.ts` (near-pure — only `@agentlet/protocol` types), `server-mount.ts`, `daemon-supervisor.ts` (fork / backoff / status). Every host thread they carry is turned into a constructor arg so the package stays free of Sediment layout / canvas knowledge:
  ...
  - **`AcpAgentletStatus`** — the `getStatus()` / `restart()` return type (also consumed by the browser via `/api/acp/daemon`) moves from `@sediment/shared` into [`@agenetes/protocol`](../../external/agenetes/packages/protocol) as a wire type (zod; web `import type`).

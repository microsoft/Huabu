...
**Progress snapshot (2026-07-05).** M1 (contracts authored) · M2 · M2.5 · M2.6 · M3 · M4 are **shipped** — each landed behaviour-preserving with the full `apps/server` suite (277) + workspace typecheck green. **Next up: M5** (extract the ACP driver's host couplings into injectable ports so the standard driver ships inside the package). **M6** (re-split `@agentlet/server` per [§6.1](#61-re-splitting-agentletserver-transport-vs-control-plane) + the lower-seam lifecycle ARI) follows; **M7** (generalise the RFS resource shape) stays deferred until a real second consumer appears. See each milestone's inline **Status** tag for detail.
...
- *Goal:* re-split the `@agentlet/server` that M4 wrapped *whole* inside `@agenetes/agentlet-host` — leave transport in `@agentlet/server`, move the control plane into Agenetes (the [§6.1](#61-re-splitting-agentletserver-transport-vs-control-plane) debate M4 deliberately shelved).

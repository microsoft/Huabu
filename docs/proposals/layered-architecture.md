...
**M4 — Cover agentlet behind one L2 facade + unify the connection token.** *(Re-scoped: the L1↔L2 seam closure on the transport side — L1 stops seeing agentlet directly; the ephemeral per-fork daemon token becomes an L1-owned global connection token injected down.)*
- *Goal:* close the L1↔L2 seam on the transport side — L1 (`app.ts`) stops directly importing the agentlet/ACP transport internals (`mountAgentletServer`, `getDaemonSupervisor`, `getDaemonAuth`) and reaches everything agentlet-shaped through **one** L2 facade entrypoint, `mountAgenetes(app, { connectionToken })`. Per the standing principle, the browser↔server HTTP surface is **not** part of this seam and does not move.
- *Why now — and why this is not the old "extract ACP host ports" M4.* After M2.5 (empty host-agnostic `@agenetes/runtime`) + M2.6 (handle holds only `threadId`; `ensureAcpSession` lives in the shell) + M3 (port-injection), **there is no upward-import layering violation left inside the ACP module** — both drivers are legitimate host-side injected objects. What *does* still leak is coarser and concrete: `app.ts` (the L1 composition root) still names the agentlet transport internals directly. So the real remaining L1↔L2 work is to *hide agentlet wholesale behind an L2 facade*, not to extract driver ports (that relocatability work moves to M5/M6).
- *Two distinct interfaces, kept apart (the recurring correction).* The `/api/acp/*` REST routes (profiles / daemon status / agent-cli list / thread control) and the `/api/rfs/:canvasId/*` routes are **L1's own web / canvas-data transports** (browser↔server + agent↔canvas reachback — RFS is canvas-coupled: node revisions, artifacts, skill guide) — they **stay in L1**, registered explicitly by `app.ts`. Only the *agentlet transport plane* — the WSS upgrade endpoint agentlet children connect back on (`mountAgentletServer`), the fork/attach supervisor (`getDaemonSupervisor`), and the handshake auth — moves behind the facade.
- *The token — an independent, complete item (dissolves the RFS↔agentlet coupling).* Today the handshake token is minted *ephemerally per fork* by `getDaemonAuth()` and rotated on every re-fork; the RFS Bearer gate has to reach that same acp-internal singleton to validate agent reachback, which is the *only* thing binding the (canvas-coupled, L1) RFS route to the agentlet transport plane. M4 promotes it to a **global, non-ephemeral `connectionToken` in L1 config**, passed *down* into `mountAgenetes(app, { connectionToken })` and forwarded to the forked agentlet children (env / argv) for their handshake. The dependency arrow becomes clean and one-directional: L1 owns the token (config source); the RFS gate validates against its *own* config value (zero L2 dependency, no `getDaemonAuth` reach); the agentlet handshake validates the same injected value. No shared singleton, no per-fork lifecycle coupling — so two unrelated APIs are no longer forced together. Changing the token source is a self-contained item (it stops being "spawn-time ephemeral" and becomes "config-global"), which is why it lands whole here rather than half-done.
- *Deliverables:* (1) an L1 `connectionToken` config option (global, stable); (2) the `mountAgenetes(app, { connectionToken })` facade — absorbs `mountAgentletServer` + `getDaemonSupervisor().attach` + the handshake-auth token source + the M3 `installAcpProfileCachePort()` wiring + the legacy `acp-config.json` cleanup, exposing one call to `app.ts`; (3) rework `getDaemonAuth` (or its successor) to take the token from the injected config rather than minting per fork, and forward it to the agentlet child; (4) point the RFS Bearer gate at the L1 config token directly. Afterwards `app.ts` names no agentlet/ACP transport internal — only `mountAgenetes(...)` plus its own `/api/acp/*` + `/api/rfs/*` route registrations.
- *Deferred (explicitly not this milestone):* extracting the ACP driver's `ensureAcpSession` host couplings into injectable ports (→ M5); persistence relocation + the agentlet-internal transport/control re-split ([§6.1](#61-re-splitting-agentletserver-transport-vs-control-plane)) + physical relocation of the ACP driver into `@agenetes/runtime` (→ M6). The built-in agent stays a host-injected custom driver throughout.
- *Depends on:* M3.
- *Acceptance:* `app.ts` imports no agentlet/ACP transport internal (`mountAgentletServer` / `getDaemonSupervisor` / `getDaemonAuth`) — only `mountAgenetes(app, { connectionToken })`; the `/api/acp/*` and `/api/rfs/*` routes stay registered by L1; the RFS Bearer gate validates the L1-config `connectionToken` with no reach into acp internals; forked agentlet children authenticate with that same config token; the seam lint (M0) shows the arrow is L1→L2; behaviour unchanged; full `apps/server` suite green.

...
- **Upward to L1** — the request (in) / `AgentStreamEvent` (out) pair. The **output** is uniform: L1 renders a stream of ~14 event types with *zero* awareness of pi-agent-core or ACP internals ([useAgentStream.ts](../../apps/web/src/hooks/useAgentStream.ts)), so it cannot tell the drivers apart *from the stream*. The **input** is per-driver, not a shared envelope ([§3.6.1](#361-dispatch-driver-affinity-discovery--and-why-l2-is-not-a-scheduler)): each driver owns its `request` type (the internal driver's is `ChatEnvelope`-shaped; the ACP driver takes `session/prompt` blocks), reflecting the Input column above.
...
//     A tagged union keyed on the top-level driver route `kind` (§3.6.1); each
//     driver member owns its `spec` (create config) AND `request` (per-turn payload).
type WorkloadSpec =
  | { kind: 'internal'; workloadKind: WorkloadKind; threadId: ThreadId; spec: BuiltinAgentSpec; request?: ChatEnvelope }
  | { kind: 'external'; workloadKind: WorkloadKind; threadId: ThreadId; spec: AcpSessionSpec;   request?: AcpRequest };
...
interface AgentHandle<Req = unknown> {
  submit(input: Req): void;                        // data plane in — the driver's own request type (§3.6.1)
...
```ts
// The route (`kind`) is a top-level, public discriminant. Each driver contributes
// one member carrying its OWN spec (create-time config) AND request (per-turn payload) —
// there is no universal request shape (attachments/canvasContext are canvas-driver-specific).
type WorkloadSpec =
  | { kind: 'internal'; workloadKind: WorkloadKind; threadId: ThreadId;
      spec: BuiltinAgentSpec;  request?: ChatEnvelope }   // canvas driver's request (content + canvas ctx)
  | { kind: 'external'; workloadKind: WorkloadKind; threadId: ThreadId;
      spec: AcpSessionSpec;    request?: AcpRequest };     // ACP driver's request
...
// is OPTIONAL — connect first, then submit(request) as runtime traffic.
```

**Two orthogonal top-level discriminants — and the earlier rename is what lets them coexist.** The **driver route** (`kind: internal/external` — *which* driver, its `spec`/`request` *types*) and the **workload kind** (`workloadKind: Job/Deployment` — completion semantics, [§3.2](#32-workload-kinds-job-vs-deployment), which only decides whether `request` is required) are independent axes. Both now sit at the top level; they do **not** collide only because the lifecycle field was deliberately renamed `workloadKind`, **not** `kind` — reserving the bare `kind` for the already-shipped, persisted driver-route value ([acp.ts](../../packages/shared/src/types/api/acp.ts) `agentBindingSchema`), whose `internal`/`external` values stay stable.

**`request` belongs to the driver, not to a universal envelope — the same schema `defineBinding` declares is reused by `submit`.** There is no driver-agnostic request: `attachments`, `canvasContext`, `intentData`, `anchorNodeId` only carry meaning under the canvas (internal) driver ([§3.7](#37-walkthroughs--the-model-against-todays-code) shows today's wire hoists exactly these to the top level — the "internal-driver bias" this union corrects). So each driver owns both its create-time `spec` **and** its per-turn `request`; the very same per-driver `request` schema types both the union member above and that driver's `submit(request)` signature ([§3.6.2](#362-the-upward-interface--agenthandle-the-duplex-control-channel-and-capability-negotiation)) — one definition, two use sites, no duplication.

**The contract ships the building blocks; the host composes the closed union.** Per [§5](#5-inter-layer-contracts-the-seams), the upstream `@agenetes/protocol` package does **not** hard-code `internal`/`external`; it exports `defineBinding({ kind, spec, request })`, a `composeWorkloadSpec([...])` combinator, `workloadKindSchema`, and the branded `threadId`/`sessionId` ids. `apps/server` (the host — it knows every driver it registers) calls `composeWorkloadSpec` over its two `defineBinding` results to build the closed `discriminatedUnion('kind', …)`; the web bundle imports the inferred type as `import type` only (stays zero-zod). Because the server *is* the host and knows all its drivers, validation at the trust boundary is a single typed pass — no `z.unknown()` two-phase is needed on this seam (that would only be for a generic proxy forwarding unknown bindings, which this conduit is not).
...
```ts
interface AgentHandle<Req = unknown> {  // core — every driver implements this
  submit(input: Req): void;             // Req = the driver's own request type (§3.6.1), not a universal envelope
...
L1 ── WorkloadSpec (top-level `kind` route + workloadKind + per-driver spec/request) ─▶ L2   (kind selects the driver; workloadKind = Job/Deployment, §3.2/§3.6.1)
L1 ── submit(request) ────────────▶ L2      (data plane in; per-driver payload — internal driver's is ChatEnvelope-shaped, §3.6.1)
...
**Wire-contract discipline (restated for the seams, with one carve-out).** Every seam type is defined once, validated on the server via `safeParse`, never redefined inside `apps/server` or `apps/web`, and imported into the web bundle as `import type` only (keep the bundle zod-free). **Carve-out:** the driver-agnostic L1↔L2 *protocol* primitives (the `WorkloadSpec` building blocks, `AgentStreamEvent` mirror, `ControlMsg`, `AgentCapabilities`) live in the extractable [`@agenetes/protocol`](../../external/agenetes/packages/protocol) package rather than `packages/shared` — they are the reusable control-plane contract, not Huabu-specific wire types. Host-specific pieces (the internal driver's `spec`/`request`, e.g. `BuiltinAgentSpec` / `ChatEnvelope`) stay under `packages/shared` and are wired in via `defineBinding`. This deviation from "wire types live in `packages/shared`" is recorded in [api-design.md](../architecture/api-design.md).
...
- *Deliverables:* the driver-agnostic protocol primitives as zod schema + `z.infer` types in [`@agenetes/protocol`](../../external/agenetes/packages/protocol) ([§5](#5-inter-layer-contracts-the-seams) carve-out) — `defineBinding({ kind, spec, request })` + `composeWorkloadSpec([...])` (the host composes the closed `WorkloadSpec` union keyed on the **top-level `kind`** route; each driver owns its `spec` + `request`, [§3.6.1](#361-dispatch-driver-affinity-discovery--and-why-l2-is-not-a-scheduler)), `workloadKindSchema`, the branded `threadId`/`sessionId` ids, `AgentStreamEvent` (a zod mirror of the ~14 existing types), the **field-level `ControlMsg`** vocabulary (up/down, capability-gated — resolves that [§8](#8-open-questions) detail), and the composable `AgentCapabilities` descriptor + its **granularity list**; the host-specific driver `spec`/`request` (e.g. `BuiltinAgentSpec` / `ChatEnvelope`) stay in `packages/shared` and bind in via `defineBinding`; the discovery/offering catalogue shape; pin the **two-level identity contract** ([§8](#8-open-questions)): `threadId` = L1-minted caller/slot key (the only wire-addressed id), `sessionId` = L2-internal execution instance surfaced only via capability-gated query/resume, with resume as a `loadSession`-gated `Deployment` capability.
- *Depends on:* M0.
...
**M2 — Introduce `AgentHandle` + the driver registry.** *(Behaviour-preserving abstraction.)*
- *Goal:* make the latent common interface ([§3.6.2](#362-the-upward-interface--agenthandle-the-duplex-control-channel-and-capability-negotiation)) explicit without changing behaviour.
- *Deliverables:* define `AgentHandle` (methods facet + async-iterable events facet + injected-ports facet); `AcpAgentHandle` wraps [`AcpAgentClient`](../../apps/server/src/modules/agent/acp/client.ts), `BuiltinAgentHandle` wraps the pi-agent-core harness; a driver registry keyed by the top-level `kind`; move definition-resolution + state-rebuild *into* `create(spec)` (fixes control-plane leak #4).
- *Depends on:* M1.
...
**M3 — Collapse control ops onto one duplex `control()` channel.**
- *Goal:* replace the side-band, ACP-shaped REST routes with one Huabu-owned control channel ([§3.6.2](#362-the-upward-interface--agenthandle-the-duplex-control-channel-and-capability-negotiation)).
- *Deliverables:* fold `/acp/threads/:t/{mode,model,commands,permission}` into the `control()` vocabulary (leaks #1–#3); reverse permission = injected `onPermissionRequest` port; out-of-turn pushes (`available_commands_update`) become stream-delivered, not REST-polled; keep the browser hop as the SSE↓ + POST↑ bridge (no UI-visible change yet).
- *Depends on:* M2.
- *Acceptance:* permission / mode / slash flows work end-to-end through the new channel; the UI is unchanged or minimally adapted; tests pass.

**M4 — Dependency-invert the built-in canvas coupling.** *(Decision (c) + built-in = L1.)*
- *Goal:* remove the compile-time `canvas-executor` coupling while keeping the call in-process.
- *Deliverables:* replace [`canvas-write.ts`](../../apps/server/src/modules/agent/tools/handlers/canvas-write.ts)'s hard `import` of `executeOnServer` with an injected `canvasPort`; L1 injects the **built-in driver factory** (closing over prompt + tools + `canvasPort`) into L2's registry ([§4](#4-layer-3--task-automation--task-driven)).
- *Depends on:* M2.
- *Acceptance:* `modules/agent` no longer imports `canvas-executor`; built-in tool calls stay in-process (zero serialization); the seam lint (M0) proves the arrow is L1→L2; tests pass.

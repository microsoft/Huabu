...
**Progress snapshot (2026-07-05).** M1 (contracts authored) · M2 · M2.5 · M2.6 · M3 · M4 are **shipped** — each landed behaviour-preserving with the full `apps/server` suite (277) + workspace typecheck green. **Next up: M5** (relocate the standard ACP driver *and its session management* into L2 as a new `@agenetes/acp-driver` package, reaching transport intra-L2; L1 injects only a storage path + the render closure). **M6** (re-split `@agentlet/server` per [§6.1](#61-re-splitting-agentletserver-transport-vs-control-plane) + the lower-seam lifecycle ARI) follows; **M7** (generalise the RFS resource shape) stays deferred until a real second consumer appears. See each milestone's inline **Status** tag for detail.
...
**M5 — Relocate the standard ACP driver (with its session management) into L2 as `@agenetes/acp-driver`.**
- *Reframing (corrects the earlier "inject host couplings as L1 ports" framing).* The earlier plan treated session management + persistence as L1 host modules to be injected into L2 as ports. That inverts the layering: **session lifecycle + session persistence are L2 responsibilities** ([§3.2](#32-workload-kinds-job-vs-deployment) / [§3.4](#34-control-plane-vs-data-plane-and-where-session-state-lives)), so M5 is a **relocation** — the standard ACP driver *and its session management* move *into* L2 — not a port-injection exercise. L1 injects only what is genuinely host-specific: a storage root path and the canvas-coupled render closure.
- *Goal:* let the standard (ACP) driver ship *inside* L2 (M2.5's K8s/CRI end-state — the analogue of a built-in container runtime), reaching its transport as an intra-L2 dependency rather than an L1 hand-down. The built-in driver stays a host-injected custom driver (canvas-coupled, L1-owned).
- *New package `@agenetes/acp-driver`.* The standard ACP driver cannot live in `@agenetes/runtime` (whose acceptance is "depends only on `@agenetes/protocol`" — a pure, empty framework); pulling in `@agentlet/server` + fastify would break that purity. So it gets **its own L2 package**, `@agenetes/acp-driver`, depending on: `@agenetes/protocol` (contracts) + `@agenetes/runtime` (framework) — the **shared base** any driver builds on — plus `@agenetes/agentlet-host` (its ACP-specific transport, an **intra-L2** dependency; a future non-agentlet driver would not touch it). Target package layout:
  ...
  ```
  `protocol` + `runtime` are the shared base (a second driver reuses them, peer to the ACP driver); `agentlet-host` is private to the ACP driver, not shared base.
- *Relocates into `@agenetes/acp-driver` (all L2 state / logic):* `AcpAgentHandle` (execution), `AcpAgentClient`, the `session/update → AgentStreamEvent` translator, the live `session-registry` (in-memory L2 session state), the `session-store` (sessionId persistence for `session/load` recovery — L2 state), and the `ensureAcpSession` orchestration (get-or-create + inflight-dedup + stale-evict + meta seed/replay), plus the `acpAgentDriver` registration object. Separately, a **driver-agnostic turn-history seam** (get-turns / append-turn by `threadId`, backed by the injected storage path) lifts into `@agenetes/runtime` (the framework, since it serves every driver) — see the transcript boundary resolution below.
- *L1 injects only two things (the minimal, genuinely-host surface):* (1) a **storage root path** (`dataDir`, L1 deployment-layout config — same idiom as M4's `mountAgenetes({ dataDir })`; the ACP session store owns its own read/write logic and just needs the root, with **`canvasId` as a legitimate path segment** — a namespace key, *not* canvas semantics leaking into L2); (2) the **render closure** (per-turn `envelope → ACP wire blocks`; the canvas-coupled `prepareExternalAgentPrompt` is genuine L1 semantics and rides in via the closure, as it already does since M2). Transport is **not** injected — it is `@agenetes/agentlet-host`, reached intra-L2.
- *Also:* make `WorkloadSpec` fully serializable (snapshot the spawn recipe into the spec so `create` does not depend on a live `getProfile()` lookup). ~~Drop `canvasId` from `loadTurns`~~ — **dropped from scope**: `canvasId`-as-path-key is legitimate namespacing, not an identity redundancy, so there is nothing to clean up here. Optionally relocate `intent` to an L1-owned module (cosmetic, [§8](#8-open-questions)).
- *Boundary resolution — `profile-store` stays L1 (confirmed against the code).* [`profile-store.ts`](../../apps/server/src/modules/agent/acp/profile-store.ts) is a file-backed CRUD of "user-configured external agent **profiles** … long-lived spawn recipes (cli + cwd + flags) which the **UI uses as templates**", edited in Settings. That is the K8s *task-example / manifest* role — human-authored example specs that live *outside* the control plane — so it stays L1; the recipe is snapshotted into the serializable `WorkloadSpec` at spawn time, so the driver never does a live `getProfile()` lookup.
- *Boundary resolution — the conversation transcript is a driver-agnostic **L2 history interface**; UI rendering + context assembly stay L1.* A code comparison settles that [`chat-thread-store.ts`](../../apps/server/src/modules/agent/store/chat-thread-store.ts) and agentlet's [`EventStore`](../../external/agentlet/packages/server/src/event-store.ts) are **orthogonal** concerns: the EventStore is per-`sessionId`, holds **raw ACP JSON-RPC wire frames** (both directions), lives and dies with one ACP session, and exists **only for ACP sessions** — a transport-level replay log. `chat-thread-store` is per-`threadId`, holds the **structured `ChatEnvelope` + assistant/tool turns** (a semantic conversation record), **outlives** any single session (survives agent rebind / `session/load` / session rebuild), and is **driver-agnostic** — the built-in path uses it too, and derives its pi-ai `Context` from it. Because it is driver-agnostic *turn persistence*, the **history/persistence mechanism belongs in the L2 framework** (`@agenetes/runtime`), not in any single driver and not in L1: M5 lifts a driver-agnostic turn-history seam ("get turns for `threadId`" / "append a turn", backed by the L1-injected storage path) into the runtime. What stays L1 is the *consumption*: the UI rendering of that history and **context assembly** (`envelope → pi-ai Context`, layering system / skills / attachments — the L1 sense-making of [§8](#8-open-questions)), plus the per-turn overlay-presentation enrichment (`applyToolExt`). L1 is a *consumer* of the L2 history interface.
- *Deferred to M6 — do not drop agentlet's EventStore in M5.* Once L2 owns a driver-agnostic history, agentlet's per-session EventStore becomes redundant and agentlet can converge to a **stateless transport** (the model the built-in driver already uses: the host holds history and re-feeds context each turn, rather than the agent self-restoring via `session/load`). That is the elegant end-state, but the convergence is a **paradigm shift, not free**: resume would move from `session/load` (agent restores its own memory) to **L2 replaying its held history into the agent**, and an external agent's live state is not only the message transcript (tool state, MCP connections). So M5 **keeps** the EventStore and preserves today's `session/load` behaviour (zero regression); dropping it + making agentlet stateless is folded into M6, which is already the "re-position the agentlet↔agenetes package boundary" topic ([§6.1](#61-re-splitting-agentletserver-transport-vs-control-plane)).
...
| L1→L2 | [apps/server/src/app.ts](../../apps/server/src/app.ts) (`mountAgenetes`)                     | In-process composition root; the single L2 mount facade.   |
...
| L2    | [external/agenetes/packages/agentlet-host](../../external/agenetes/packages/agentlet-host)   | Agentlet transport host (`mountAgenetes`) — M4.            |

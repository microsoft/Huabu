...
| Small state/observation edge table | First user content, current-result acknowledgement and untracked reporting cross existing APIs                                         | Specify first-submission detection before replacing the invocation token, legacy-empty-node behavior, stale ack behavior, and the existing read response that exposes untracked execution. Define driver terminal precedence from the supported event contract rather than from receipt of an HTTP end                                                    |

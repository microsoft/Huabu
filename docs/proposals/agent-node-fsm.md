...
On reads and SSE updates, merge authoritative FSM metadata even when local authored content is dirty. Preserve the unsaved content, but do not keep a stale local Editing/running value simply because the current sync filter skips a whole node replacement. This is read-side reconciliation, not another state owner.
...
The user's subsequent field-ownership decision refines this evaluation: separate node read models from owner-specific write contracts, rather than retaining unrestricted snapshots and adding compensating rules to every writer. The code baseline is unchanged; the target design and effort interpretation below incorporate this simplification.
...
| Field ownership            | Define narrow editable payloads separately from FSM transitions and result acknowledgements. Reuse those contracts across PUT/commands and project ordinary undo onto editable effects. Compose edits with current server state instead of repairing arbitrary whole-node replacements.                                                         |
...
| Untracked execution        | Preserve existing observation, error and runtime-admission behavior without a repair-on-GET or replay loop. Do not require a new unknown-state UI or retry policy; escalate only a concrete incompatible case.                                                                                                                                  |

### Source-to-target gap matrix

Sizes below are relative integration effort, not calendar estimates. Multiple writers still require coordinated migration, but the owner-specific contract removes the need for a large per-entrypoint anti-overwrite mechanism.
...
| Canonical Canvas writers: `canvas.route.ts:1155-1283`, `canvas-executor.ts:674-709,1115-1248`                                                                | Structure PUT accepts metadata wholesale with version CAS; executor/inverse replay use Canvas mutex, but PUT is not enclosed by it                                                   | Narrow editable payloads, compose them with current server fields, and constrain ordinary inverse replay to editable effects; reuse a shared ownership contract and local ordering | Medium, cross-path migration    |
...
The design is feasible with existing Huabu/Agenetes services. Owner-specific writes reduce ongoing complexity: ordinary edits, FSM transitions and acknowledgements no longer compete to replace the same fields. This replaces the earlier framing of Canvas writes as a large defensive anti-overwrite mechanism. Initial work remains a cross-layer migration of payloads, server composition, undo and browser observation; it is not just adding an enum, but does not require a new storage or permissions framework. Draft-edit versus first-binding ordering and current-invocation guards remain necessary local rules. No Agenetes protocol change is currently required; internal realization needs a Huabu-side integration seam.

1. Resolve the frozen-field and node-identity questions above; define the owner table and separate shared read/edit/transition/ack contracts.
2. Narrow ordinary Canvas saves/commands and undo effects, then wire binding confirmation and initial draft persistence through those contracts; include existing-node/copy/fork/attachment handling.
...
5. Preserve existing state presentation and interaction while completing core-path regressions and low-cost caller smoke coverage; update architecture with shipped behavior. New untracked-state feedback is not a deliverable. Do not block the core migration on Task/Run/View edge cases.
...
## 9. Readiness checkpoint after owner-specific writes

Reviewed on 2026-09-14 against runtime baseline `6e69502c` (documentation-only commits since the section 8 evaluation). The architecture is sufficiently settled to stop broad investigation. The document is not yet a fully specified implementation contract: the remaining work is a bounded contract pass, not another architecture or recovery project. This checkpoint does not authorize runtime implementation.

### Open-question disposition

| Item                                          | Disposition                                                                                                                                                                                                        |
| --------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Binding evidence, storage and irreversibility | Resolved: validated ThreadRecord confirms Bound; node metadata persists the monotonic acknowledgement; Session IDs are not involved                                                                                |
| Field ownership and whole-node PUT            | Resolved direction: distinct read/edit/transition/ack models; no unrestricted client replacement of FSM metadata                                                                                                   |
| Task/Run/View                                 | Resolved scope: low-cost happy paths only, not a readiness gate                                                                                                                                                    |
| Job/Deployment                                | Preserve existing strategy; a valid threaded Job record is evidence, not a new kind of node or a reason to add a framework                                                                                         |
| Copy/fork/attach/move                         | Use the identity rules in section 8 without adding a new UX: clear current-invocation metadata for a new thread, confirm fork/attachment through that thread's record, preserve Bound for the same binding on move |
| Frozen preparation fields                     | Binding identity and explicit launch overrides are settled. Internal `agentMode` needs a concrete compatibility rule in the contract pass below; do not freeze all node metadata                                   |
| Recovery                                      | No automatic recovery requirement. Observation may report uncertainty; do not infer success or permit duplicate execution on a guess                                                                               |

For `agentMode`, the conservative implementation recommendation is to preserve existing behavior rather than introduce a new post-bind mode-switch feature. Current node-backed ChatPanel chooses a saved replay mode (`index.tsx:146-150,197-200`), whereas internal dispatch receives a per-invocation scope (`agent-thread.service.ts:516`). Those are not a universal Agenetes binding invariant. Keep this field outside driver/Profile identity comparisons; retain existing command/UI affordances and define when its initial selection is saved. Changing the supported ask/operate UX would require a separate explicit decision.

### Final contract pass before runtime changes

Field-contract review on 2026-09-14: the user agreed to the field ownership table and these edit semantics: ordinary requests contain editable fields only; omission means unchanged, including server-owned fields; clearing is explicit and field-specific (for example empty authored text or a supported launch-override reset), never a blanket conversion of `undefined` to an empty string. Internal ask/operate behavior remains unchanged and outside execution-binding identity. This settles the business rules for the first review item; concrete shared schemas still need to encode them. It is not runtime implementation or publication approval.

| Required deliverable               | Why it is still concrete work                                                                                                          | Bounded completion criterion                                                                                                                                                                                                                                                                                                                              |
| ---------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Field list and payload shapes      | Existing `putCanvasBodySchema.state` and command elements are `unknown`; prose about owners alone does not implement a writable subset | Enumerate the affected Agent Node edit fields, omitted-versus-clear semantics and transition/ack inputs in shared contracts. Specify explicit rejection for forbidden client writes, initial creation/attachment behavior, and how existing snapshot undo is projected onto editable fields. Do not enumerate a new permission system for every node type |
| Local transition sequence          | Draft edits, first realization, Canvas promotion, cancellation and move already use different coordination paths                       | Write one actual sequence for prompt, control and draft edit, including internal create confirmation, no recursive Canvas lock, and no turn wait while holding Canvas locks. Use interleaving tests during implementation; do not choose a new mutex hierarchy solely from the diagram                                                                    |
| Small state/observation edge table | First user content, current-result acknowledgement and execution observation cross existing APIs                                       | Characterize existing first-content, legacy-node, stale-ack and driver terminal behavior, then encode the compatible server-owned path. Preserve existing UI and error surfaces rather than defining a new untracked status or interaction.                                                                                                               |

First-submission detection can reuse the prior current-invocation marker for migrated nodes and existing history evidence for a legacy empty node; it does not require another journal. A control-only Bound node still has no admitted prompt. The current ChatPanel fallback for an empty thread whose Profile was removed (`index.tsx:251-280`) must also use binding evidence rather than message count: a control-only Bound node must not silently switch to the internal Agent.

The above are implementation-design deliverables the engineer can resolve without another round of broad product questions. If one cannot preserve the stated behavior, surface that specific choice rather than silently changing it. Once they are written, the plan can be approved for one coherent server/Web implementation; neither publication of this proposal nor this readiness check grants that approval.

Third-item review outcome, 2026-09-14: the user accepted proceeding with state/observation rules subject to preserving existing presentation and interaction as far as possible, with no new definitions in this PR. This supersedes earlier recommendations that made explicit new untracked/unknown UI feedback or a new follow-up restriction mandatory. Remaining terminal/first-content/ack details are compatibility characterization and implementation contracts, not an invitation to define another status or recovery workflow. This design agreement does not authorize runtime implementation, commit/push, or merge.

## 10. Final pre-implementation review

Reviewed on 2026-09-14 after the three contract discussions. The agreed product and architecture direction is ready for implementation approval: owner-specific writes, monotonic binding confirmed at Agenetes create time, local preparation/admission ordering, and preservation of existing state presentation and interaction. Section 9's earlier readiness assessment is historical; its remaining schema names, field encodings and adapter details are bounded implementation tasks, not reasons to reopen product design. A concrete deviation from the agreed behavior must still be surfaced rather than silently introduced.

**Implementation baseline is not yet refreshed.** At this review, fetched `origin/main` was `54d1e6cb`; the proposal branch had four documentation-only commits not on main and main had 47 commits not on this branch. No merge or rebase was performed. Before runtime edits, integrate current main with authorization and refresh only the affected integration points below. Passing checks on the old branch does not validate that integration.

| Main-branch development                               | Effect on this plan                                                                                                                                                                                                                                                                                                                                                                                  |
| ----------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Dirty-content delta handling (`2aec991d`, `0d12310a`) | Main already applies metadata-only REPLACE_NODE deltas while preserving pending content, using value comparisons. Reuse this implementation and `canvasStore.agentDeltaConflict.test.ts`; evaluate the remaining mixed content-plus-FSM update case instead of rebuilding the old whole-node-skip path.                                                                                              |
| ACP refresh binding fix (#182)                        | Main adds `resolveConversationAgentBinding()` and uses persisted owner binding in ChatPanel/send paths. Reuse it rather than introducing another binding-selection helper; reconcile its precedence with acknowledged Editing drafts.                                                                                                                                                                |
| Auto-accept change review (#176)                      | Canvas execution now conditionally omits review records for Agent changes. Preserve auto-accept/manual-revert behavior when separating ordinary edit undo from FSM metadata.                                                                                                                                                                                                                         |
| SQLite storage support (#92)                          | Agenetes now mounts conversation-store dispatchers for file/SQLite/in-memory backings. `agenetes.record()` remains the public confirmation seam. The physical paths in section 1 describe Disk only; persist through existing portable Canvas and Agenetes ports, not direct `space.json` or `threads.json` access. Reuse the available backend contracts rather than create storage infrastructure. |

No new Session ID coupling, recovery subsystem, Task/Run/View compatibility gate, or upstream Agenetes FSM is required by these developments. The previous gap matrix remains evidence for its stated baseline, not a claim that the listed gaps are all still missing on current main.

The final review found no blocking contradiction in the pending documentation changes. The remaining pre-code gate is refreshing the integration baseline; after that, implement the agreed scope with existing behavior characterization and targeted regression additions. Do not delete legacy fields or orphan features merely to simplify the patch. Proposal publication does not authorize runtime implementation, branch integration, merge or release.

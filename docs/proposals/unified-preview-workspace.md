...
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| L1  | `threadId` is still the globally current thread. The `selectCurrent*` accessors are gone, and only `ChatPanel` reads it — to decide which session to build                  | [`chatStore.ts`](../../apps/web/src/store/chatStore.ts)                                                                                  | Retire once a tab supplies the session directly, so nothing resolves "the current thread" at all.                                                |
| L2  | **Partly done.** `binding` and `settings` now live on the thread; `lastAction` is still global compose state                                                                | [`chatStore.ts`](../../apps/web/src/store/chatStore.ts)                                                                                  | Move `lastAction` into `ChatThreadState` too; keep node-authoritative values on the node after first send.                                       |
| L3  | ~~`pendingAttachments` and `selectionAttachment` are one global staging slot~~                                                                                              | [`chatStore.ts`](../../apps/web/src/store/chatStore.ts), [`ChatInput.tsx`](../../apps/web/src/components/Panels/ChatPanel/ChatInput.tsx) | **Done.** `pendingAttachments` is thread-local; the excerpt stays shared and a send snapshots it instead of clearing it for everyone.            |
...
The `_savedCanvas*` save/restore stack and the globally current `threadId` exist for one reason: a single panel has to stash the Canvas conversation before showing a Question one. There is nothing to stash once a tab owns its target — which is why L4's own resolution says exactly that. Removing them before tabs exist would mean inventing a temporary presentation container that Stage 3 immediately replaces. They move to Stage 3.

Everything else in this stage is done: L2's binding and settings, L3, L5, L6, L7, L8 by deletion, and L9. `lastAction` is the remainder of L2 and is deferred with L1/L4, because it is persisted per Canvas and is read by `QuestionNode`, which sits outside any Chat renderer and so has no session to read from.

The acceptance criterion splits the same way. [`chatSessionIsolation.test.tsx`](../../apps/web/src/hooks/chatSessionIsolation.test.tsx) mounts two renderers under different sessions and proves all seven listed properties independent, plus that moving the store-wide `threadId` changes nothing on screen. What it cannot yet assert — two mounted `ChatPanel`s — is left as a skipped test in the same file, because `ChatPanel` is the layer that still resolves its session from the global pointer. That skipped test is the Stage 3 acceptance criterion.

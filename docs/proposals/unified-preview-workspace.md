...
| L3  | **Partly done.** `pendingAttachments` is thread-local; `selectionAttachment` is still one global slot that a send clears for every Chat                                     | [`chatStore.ts`](../../apps/web/src/store/chatStore.ts), [`ChatInput.tsx`](../../apps/web/src/components/Panels/ChatPanel/ChatInput.tsx) | Turn the selection excerpt into the shared transient `selectionContext` of §10.2: snapshot it per request, never clear it on another session's send. |

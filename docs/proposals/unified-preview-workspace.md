...
The migration is not a data-structure tidy-up. Only four fields are keyed by thread today (`messagesByThread`, `draftsByThread`, `historyLoadedThreads`, `loadingThreadIds`); everything below is a singleton that two mounted renderers would fight over. Each row must be resolved before Stage 3 mounts a second Chat.

| #   | Leak                                                                                                                                                                                      | Location                                                                                                                                 | Resolution                                                                                                                 |
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| L1  | `threadId` is the globally current thread, and `selectCurrentMessages` / `selectCurrentDraft` / `selectCurrentHistoryLoaded` / `selectCurrentIsLoading` all read through it               | [`chatStore.ts`](../../apps/web/src/store/chatStore.ts)                                                                                  | Replace with `threadsById` plus explicit session descriptors; selectors take a `threadId` argument.                        |
| L2  | `agentBinding`, `chatSettings`, `lastAction` are global compose state                                                                                                                     | [`chatStore.ts`](../../apps/web/src/store/chatStore.ts)                                                                                  | Move into `ChatThreadState`; keep node-authoritative values on the node after first send.                                  |
| L3  | `pendingAttachments` and `selectionAttachment` are one global staging slot, written by `ChatInput` and by the node preview's `selectionchange` listener                                   | [`chatStore.ts`](../../apps/web/src/store/chatStore.ts), [`ChatInput.tsx`](../../apps/web/src/components/Panels/ChatPanel/ChatInput.tsx) | `pendingAttachments` becomes thread-local; the selection excerpt becomes the shared transient `selectionContext` of §10.2. |
| L4  | `viewingQuestionThread`, `viewingSketchCluster`, `_savedCanvasThreadId`, `_savedCanvasBinding`, `_savedCanvasLastAction` encode "what the one panel is showing" plus a save/restore stack | [`chatStore.ts`](../../apps/web/src/store/chatStore.ts)                                                                                  | Retire. Presentation moves to the tab target; there is nothing to stash and restore once each tab owns its own target.     |
| L5  | `useAgentStream()` takes no arguments and reads the global `threadId`                                                                                                                     | [`useAgentStream.ts`](../../apps/web/src/hooks/useAgentStream.ts)                                                                        | Accept an explicit session descriptor captured at mount.                                                                   |
| L6  | `useChatHistory(setIsLoading)` reads the global `threadId` and calls `switchToCanvas` as a side effect                                                                                    | [`useChatHistory.ts`](../../apps/web/src/hooks/useChatHistory.ts)                                                                        | Accept an explicit `{ threadId, canvasId }`; the Canvas switch moves to the Canvas load path.                              |
| L7  | `useAcpSessionMeta` and `useBuiltinThreadSettings` already accept explicit `threadId`, but `ChatPanel` feeds them the global value                                                        | [`ChatPanel/index.tsx`](../../apps/web/src/components/Panels/ChatPanel/index.tsx)                                                        | Feed the renderer's own session descriptor. No hook signature change needed.                                               |
| L8  | `intentStore._setOnIntentChosen` is a single callback slot registered by `ChatPanel`; a second instance silently wins                                                                     | [`intentStore.ts`](../../apps/web/src/store/intentStore.ts)                                                                              | Key the handler by session, or move intent resolution out of the renderer.                                                 |
| L9  | `panelStore.focusChatInputNonce` is one global nonce, so a focus request cannot address a specific group                                                                                  | [`panelStore.ts`](../../apps/web/src/store/panelStore.ts)                                                                                | Address focus to a tab or group ID, per §8.                                                                                |
...
Canvas node selection and Preview text selection are shared ephemeral context, not per-thread state. Every visible Chat may display the same current selection hint. On send, the submitting session snapshots the then-current selection into that request; without a send, selection changes remain UI-only and are not written into any thread. Because focusing a composer can collapse the browser selection, Preview text selection is retained in one transient `selectionContext` until the originating selection changes or becomes invalid. User-added files, captures, and other explicit attachments remain in the submitting thread's `pendingAttachments`.
...
Stages 1 and 2 are invisible to users and ship without a flag. Stage 3 puts the workspace behind a flag whose off state renders the existing `ChatPanel` and `ExpandedNodePanel`; both paths read the Stage 2 store, so flipping the flag changes presentation only and never strands state. Stage 4 migrates entry points with the flag still available. The flag is removed at the start of Stage 5, which is the point of no return.
...
| [`apps/web/src/store/intentStore.ts`](../../apps/web/src/store/intentStore.ts)                                       | Single intent-callback slot registered by the Chat renderer (L8).                            |

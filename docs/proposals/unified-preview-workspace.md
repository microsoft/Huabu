...
The acceptance criterion split the same way while Stage 2 was landing. [`chatSessionIsolation.test.tsx`](../../apps/web/src/hooks/chatSessionIsolation.test.tsx) now mounts two real `ChatPanel`s under different sessions and proves their conversations and compose modes remain independent, plus that moving the store-wide fallback `threadId` changes nothing on screen.

...
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| L1  | ~~`threadId` is the globally current thread, and `ChatPanel` reads it to decide which session to build~~                                                                                                                                                                         | [`chatStore.ts`](../../apps/web/src/store/chatStore.ts)                                                                                  | **Done for the workspace path.** `ChatPanel` takes a session; a tab supplies one from its own target. The field remains only for the single-panel fallback and goes with it in Stage 5. |
...
| L4  | **Partly done.** A tab-supplied session carries its own `conversationView`, so the panel no longer reads `viewingQuestionThread` to decide what it is showing. The save/restore stack and the replay pointer's `openSequence` / `openPosition` still serve the single-panel path | [`chatStore.ts`](../../apps/web/src/store/chatStore.ts)                                                                                  | Retire the stack with the single-panel path; move scroll-positioning state onto the tab.                                                                                                |
...
- Remove the store-wide `threadId`, the `_savedCanvas*` restore stack, and the rest of the Chat presentation state that the single-panel fallback still needs (L1 / L4).

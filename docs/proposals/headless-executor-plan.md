...
| 3.5 | _(Optional, deferable)_ Same-origin `BroadcastChannel('sediment-canvas-sync')`: when this tab successfully posts to `/execute`, broadcast `(version, deltas, optimisticTag)` to sibling tabs so they apply before SSE arrives. May be deferred if SSE latency on localhost proves sufficient.                                                                                                                                                        |
...
| 3.9 | Env flag `SEDIMENT_ENABLE_SYNC` (default on) gates SSE subscription; legacy refetch path remains as fallback for one release cycle.                                                                                                                                                                                                                                                                                                                  |
...
| SSE behaves differently inside Tauri                                                        | Tauri uses real `EventSource`; tested separately. `SEDIMENT_ENABLE_SYNC` flag plus log-pull fallback keeps a recovery path.                                                                                                    |

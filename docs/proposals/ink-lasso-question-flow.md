...
The owner-only settings API uses shared schemas, restricts HTTPS resource-root endpoints to Azure public-cloud resource or regional hostnames, and returns no saved plaintext keys. Custom gateways, IP literals, sovereign-cloud hosts, non-default ports, credentials, query, and fragment are rejected. SecretStore persists endpoint and key overrides as one versioned encrypted record, with serialized updates and lazy migration from the earlier split configuration. UI-saved values independently override environment values and affect the next submission without a restart. The API read model distinguishes stored values from environment fallbacks and describes configuration completeness rather than verified connectivity. Removing an override restores its environment fallback, never an obsolete legacy override, so deleting a saved key is not an off switch. The compact row discloses selected-stroke external processing, surfaces actionable validation errors, and disables all mutations when secure storage is unavailable. Image-only fallback remains unchanged. See [credential storage](../architecture/credential-storage.md) for runtime-specific persistence, migration, and acknowledgement-failure behavior.

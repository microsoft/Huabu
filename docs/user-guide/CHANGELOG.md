...
- AI 修改笔记内容后，展开编辑器顶部会显示 "AI modified this note" 横幅。
- 点击 "View changes" 可展开 Markdown 差异对比视图，显示 AI 的增删改。
- 横幅在用户编辑完所有 AI 修改过的 block 后自动消失，也可手动点击关闭。

**Notes**

- 快照采用累积策略：AI 多次编辑不会覆盖快照，diff 始终显示从用户上次完整拥有的内容到当前内容的所有变更。
- Block 级别的紫色色条（实线=AI 原始、虚线=用户已修改）仍然同时显示。

...
- 画布 store 里几条纯瞬态写入路径（ReactFlow 的 `onNodesChange` 拖动 tick、`onEdgesChange` 边选择、AI 流式入场时的透明度过渡动画）现在显式走 `_setStateNoAutosave` 静默通道，不再经过会比对结构差异、调度自动保存的 `wrappedSet` 路径。
- 之前这些每秒 60 次的 `set({ nodes })` 都会执行一次 `haveNodesChangedStructurally` 全量 diff，且每次都会重置自动保存防抖窗口 —— 拖一个节点拖得久，正经的几何提交反而要等更久才会被持久化。

**Notes**

- 用户视角无可见行为变化。这是一次纯性能与意图清理：现在"什么写入会触发保存"由调用方显式选择，而不是依赖 gate 函数事后过滤掉拖动噪声。
- 真正的几何落盘仍然由 `onNodeDragStop` 里的 `SET_NODE_GEOMETRY` engine 命令负责，它走正式 wrappedSet 路径，照常调度自动保存。
- 如果你在调试新代码时想让某条写入"安静地不触发保存"，参考 `_setStateNoAutosave` 的使用注释（store 里有详细说明）。

---

## 2026-05-25 · 修复：长按"预览回滚"超过 1 秒可能把回滚态错写入服务器

**What Changed**

- AI 变更面板上每条改动旁边的"按住预览回滚后样子"按钮，过去是直接 `setState` 到画布 store —— 走的是会触发自动保存的那条路径。如果用户按住超过 1 秒（自动保存防抖窗口），回滚后的画布状态就会被作为正式版本写到服务器，松手回到原状态时又会再触发一次写入。
- 现在预览和松手恢复都改走 `_setStateNoAutosave`，跟服务端回写 dedup 标签同一条静默通道；预览只影响本地显示，永远不会被自动保存意外持久化。

**Notes**

- 没有数据迁移成本。如果你之前因为这个 bug 出现过"AI 改动莫名被回滚了一截"，可以通过撤销恢复，或者从未受影响的设备/标签页重新加载来覆盖。
- "应用 / 拒绝"按钮的行为没有变化 —— 那两条路径本来就走正式的 engine 命令链，按下后的改动是该被持久化的。

---

## 2026-05-25 · 节点内容写入走独立通道（不再与画布版本号冲突）

**What Changed**

- 编辑器对节点正文 / 标题的修改改走独立的 `PUT /api/canvas/:canvasId/nodes/:nodeId/content` 接口，每次最多 500 ms 防抖一次，单节点串行落盘。视口拖拽、节点位置变动等"结构性"自动保存仍然走原来的 `PUT /api/canvas/:canvasId`，但请求体内不再携带 `content / label / labelSource / src / summary / keywords / provenance` 等字段。
- 重命名节点（双击标题或右键改名）现在会立即冲刷该节点的待保存内容，并在服务端返回 409 `NODE_LABEL_CONFLICT` 时即时回滚本地标题、弹出与旧版一致的占用提示对话框。
- 切换画布 / 关闭标签页前会先把所有待保存的节点内容冲刷完成，避免编辑器最后几个键击丢失。

**Notes**

- 无需手动迁移：服务端 `hydrateOneNode` 仍会从 `.md` 中恢复 `label` 与 `labelSource`，历史 `canvas.json` 中冗余的字段会在下一次结构保存时被自然清理。
- 部署期间如果某个浏览器标签仍在运行旧版本前端，对应标签上的最后一次重命名 / 内容编辑可能会被丢弃（旧前端仍依赖已下线的 `renamedNodes` 字段做对账）。建议部署后刷新一次仍然打开的旧标签页。
- 之前由 `persistAndStripNodes` 在结构 PUT 中做的批量 strict-rename 预校验已下线；同等效果现在由前端的"重命名时强制冲刷 + 等待"流程保证，AI 批量改名仍走 lazy-dedup。

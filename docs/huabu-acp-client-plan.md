...
│   └── permission.ts     session/request_permission → UI 弹窗              ✅ 实现在 client.ts（per-turn notifier + resolvePermission）
...
| `session/request_permission` | Phase 3 PR H + I **已实现**：per-turn notifier 挂起 + SSE `permission_request` + `POST /threads/:id/permission` 回传 + 内联 PermissionCard UI | 透传 agent 给的全部 options；无 UI 承接时 auto-allow first option；5 分钟超时 → cancelled |
...
- **permission handler 拆 H + I**：原计划先后端骨架（PR H）、后真 UI（PR I），实际 **H + I 一并落地**（见下方 PR 表更新）。最终实现没有沿用「auto-allow first option」骨架，而是做成完整的挂起/恢复 round-trip：per-turn notifier → SSE `permission_request` → 用户应答 → resolvePermission 唤醒挂起的 Promise。没有 UI 承接时（warmup / 非当前轮）仍 auto-allow first option 保底。`always` 持久化暂未实现（透传 agent options，决定不落库）。
...
| **H + I** ✅ | **已合并实现（一并完成，未拆两个 PR）**：(a) `acp/client.ts` 实现 `session/request_permission` 完整挂起/恢复：per-turn `PermissionNotifier` + `pendingPermissions` Map + `resolvePermission(requestId, decision)` + 5 分钟超时 + turn/abort/shutdown 时取消挂起项；无 notifier 时 auto-allow first option；(b) `packages/shared` 加 `permission_request` SSE event（`AgentPermissionRequestEventData`）+ `AssistantPermissionPart` 装配片段 + `acpPermissionDecisionSchema` 回传契约；(c) `service.ts` 注入 per-turn notifier 把请求 push 进 SSE 队列（不持久化）；(d) `threads.route.ts` 加 `POST /threads/:id/permission` 调 `resolvePermission`；(e) web：`useAgentStream` 追加 permission 片段、`PermissionCard.tsx` 内联渲染 allow/reject 按钮 + 乐观应答 + 折叠摘要 | client.test.ts 3 个新测试覆盖 auto-allow / suspend→resolve select / cancelled + 幂等；server + web typecheck 绿；用户能在聊天里点 allow/reject |

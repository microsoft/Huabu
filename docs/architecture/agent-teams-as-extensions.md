...
`GET /api/acp/agent-cli` returns cached machine-scoped observations of the complete trusted Agent catalogue; `POST /api/acp/agent-cli/refresh` runs the same target-agentlet discovery used after connection and reconnection. The daemon reports observations keyed by the host-supplied harness id and never supplies launch recipes. Ordinary Settings creation projects those observations into installed and disabled Not installed sections and keeps Custom command last. A selected Preset filters that same catalogue to its manifest harness allowlist and blocks unknown ids. The selector does not embed installation commands; user documentation links to each Agent's official installation source so package changes do not stale the UI.
...
Chat keeps the built-in Agent separate and presents saved Profiles alongside installed machine-scoped discovered Agents. Choosing a discovered Agent explicitly materializes or reuses a normal visible command Profile with host-owned discovery provenance; refresh never mutates Profiles. Same-named harnesses on different machines remain distinct through `(agentletId, harnessId)` identity and machine-qualified display. Add agent still opens External Agents Settings for manual customization.
...
| [`apps/server/src/modules/agent/acp/agent-cli.route.ts`](../../apps/server/src/modules/agent/acp/agent-cli.route.ts) | Owner-only machine discovery, refresh, materialization, and target-native path validation adapters.                          |

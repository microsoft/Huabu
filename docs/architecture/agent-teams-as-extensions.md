...
- Every non-internal agent uses one Agenetes Agent Profile. Profile schema v2 adds first-class `resourceIds` to both command-backed and manifest-backed Profiles; Huabu composes those optional selections with its required Agent Resources when creating a workload. Manifest Profiles otherwise retain immutable placement, manifest, harness, and working-directory fields plus authoritative durable preparation state; command Profiles retain immutable placement, command, and working directory. Setup diagnostics are stored separately from the Profile CRD.
...
10. **Resources do not replace Setup** — registry file v4 migrates every Profile to schema v2 with `resourceIds`, but manifests do not implicitly publish resources and Profile selection does not bypass Config, preparation, workspace, or runtime validation. See [`agent-resources.md`](./agent-resources.md).

...
- **Sketch migration (in progress).** The sketch pipeline (`origin.type === 'sketch-recognized'`) is still applied **client-side in a single pass** and cannot read back server-assigned ids. Now that the `id` field is removed from the agent schema, sketch gestures that must self-reference a node they create in the same batch (e.g. circle-to-group's new frame) are **temporarily unsupported**; gestures that only touch existing nodes still work. The fix is to route sketch recognition through `executeOnServer` + broadcast like every other agent path, so it reads real ids from `results[].nodes` and drops the client-side apply entirely — tracked as the sketch server-apply migration.

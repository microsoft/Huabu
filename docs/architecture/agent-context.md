...
The OCR adapter implements Azure AI Vision's Image Analysis Read protocol only; its configurable endpoint does not make it a generic OCR or LLM connector. Settings uses the existing secret-entry and deployment-readiness components. UI overrides take precedence independently for the endpoint and key and apply to the next OCR call without restarting; an in-flight provider request retains the configuration it already captured. See [credential storage](./credential-storage.md) for persistence, masked status, and removal semantics.

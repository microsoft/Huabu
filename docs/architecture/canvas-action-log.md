...
- [memory/analyzer.ts](../../apps/server/src/modules/agent/memory/analyzer.ts) `readEventsDigest(canvasId)` (up to `MAX_EVENTS_IN_DIGEST = 100`) is fed to the curator LLM alongside the canvas snapshot + chat digest.

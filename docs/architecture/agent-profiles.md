...
Settings > Huabu Agent exposes the default external Profile and one optional functional-task model text field above the transitional built-in provider controls; External Agents remains the Profile-management surface. Text summaries, keywords, content/Frame titles, and conversation titles now use this Profile through independent ACP Jobs. The model value is trimmed; empty inherits the Profile's remembered model, or the harness default if none exists. A known wrapper receives the preference only through advertised live ACP model controls. Unsupported or unknown model capability is a warning, not a fabricated guarantee; unavailable values are logged and the harness default remains. Custom commands receive no generic model injection. Functional execution does not modify Profile launch configuration or interactive chat's persisted session preferences. Image/vision and other unmigrated workflows retain their existing pi-ai routing.

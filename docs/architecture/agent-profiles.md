...
New automatic structured Profiles set `options.autoApprove: true` only when discovery explicitly reports `capabilities.autoApprove: 'supported'`. The wrapper compiles the corresponding approval flags; Huabu does not compose them. Unsupported, unknown, or missing capabilities leave the option unset. Users can disable it through Edit, and later discovery/reconnection preserves that choice, including after a host restart. Compatibility command Profiles, including Windows shell-only wrappers, remain Custom and receive no injected approval flags. This creation default does not change manual creation, existing Profiles, or realized execution snapshots.
...
Settings > Huabu Agent exposes the default external Profile and one optional functional-task model text field above the transitional built-in provider controls; External Agents remains the Profile-management surface. Text summaries, keywords, content/Frame titles, conversation titles, and image labels use this Profile through independent ACP Jobs. Image labels reuse chat image resolution and its 4 MB decoded-image cap, submitting canonical image blocks alongside the existing label prompt; unavailable pixels and invalid labels fail explicitly. The model value is trimmed; empty inherits the Profile's remembered model, or the harness default if none exists. A known wrapper receives the preference only through advertised live ACP model controls. Unsupported or unknown model capability is a warning, not a fabricated guarantee; unavailable values are logged and the harness default remains. Custom commands receive no generic model injection. Functional execution does not modify Profile launch configuration or interactive chat's persisted session preferences. ACP image prompts require the Agent to advertise `promptCapabilities.image: true`; the selected harness/model must actually support vision, and its errors propagate without an internal fallback. Memory, Skill authoring, and other unmigrated workflows retain their existing pi-ai routing.

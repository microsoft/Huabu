...
Full-Canvas PUT compares incoming Agent preparation against the current node under that mutex using `changesAgentNodePreparation`. Unchanged preparation echoed by a layout save does not require execution-record confirmation or promote binding state. Actual preparation changes still acquire draft admission and validate canonical execution; explicit guarded commands and Agent interactions retain their confirmation and promotion behavior. Input schemas, server-owned field rejection, association preservation, and Canvas version checks apply to every PUT regardless of whether preparation changes.

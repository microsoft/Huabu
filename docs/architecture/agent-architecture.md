...
- [`@agenetes/agent-profile`](../../external/agenetes/packages/agent-profile) owns ordinary Profile schemas, CRUD and persistence, without Team discovery, setup or Config dependencies. `buildAcpWorkloadSpec()` reads the selected Profile and snapshots its explicit placement, command or structured harness launch, and working directory before Agenetes creation. Agentlet compiles structured harness options; legacy commands remain unchanged. Retired manifest Profiles are not selectable or compiled into executable recipes.

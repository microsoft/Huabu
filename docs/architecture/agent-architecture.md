...
- **Built-in chat is a Deployment**: `POST /api/agent` reuses one live `PiAgentHandle` per `threadId` (get-or-create by Agenetes). On restart, Agenetes supplies durable materialized history through `AgentCreateContext`; that history contains completed Tier-2 turns plus an optional read-time incomplete turn projected from the Tier-1 `turn_start` and event suffix. pi-driver lowers that history through its `materializeHistory` port and seeds the result through pi-agent-core's native `initialState.messages`. Huabu implements the port in [history-replay.ts](../../apps/server/src/modules/agent/agenetes/history-replay.ts): each durable turn is re-rendered by the _same_ renderer a live turn uses (`rebuildTurnMessages`), so a recovered thread keeps its role attribution, its `toolCall`/`toolResult` pairing, and its images as real vision parts instead of being flattened into one text blob. Two budgets bound the replay, newest-first: image bytes (older images past the cap degrade to a placeholder pointing at the `origin` node id) and total units (whole turns are dropped from the front — never split, or a `toolCall` would lose its `toolResult` — and the loss is announced to the model). Because the payload is not the durable record, the driver reports the materialized `estimatedSize` to `authorizeHistoryLoad`; the mounted `AutoRecoverPolicy` limit is the same `HISTORY_REPLAY_BUDGET` and acts only as a last-resort guard for drivers with no host materializer. The route no longer rebuilds transcript context or persists turns. The workload's `initialPreamble` is mapped to pi-agent-core's native `systemPrompt`; later prompt changes use native `set_context`. The pi driver also re-resolves the symbolic `{ type: 'host', id: 'active' }` model ref at every turn boundary.
...
- Chat renders modern ACP `configOptions` as the source of truth for session controls, using legacy model and mode lists only when no matching modern option exists. Selecting the `agent-full-access` value of a mode option opens a compact confirmation popover above and left-aligned with ChatInput before Huabu sends `session/set_config_option`; cancelling leaves the active mode unchanged without blocking the canvas behind a full-screen modal.

...
- ACP has no native seam for injecting prior assistant messages, so when native resume is unavailable the driver replays history as one explicitly labelled text block after the current canonical input and any pending preamble. The current task therefore remains the leading content used by backend session-title heuristics. The driver first projects every durable turn through `projectTextHistoryTurn` (`@agenetes/runtime`), which replaces image bodies with a short placeholder — a base64 payload carries no meaning once flattened into text, and inlining it would only inflate the payload. The _projected_ turns are what gets authorized, so the admission estimate prices the block that is actually sent.

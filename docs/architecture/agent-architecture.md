...
- **RFS agent invocation has two compatible branches**: omitting `X-Huabu-Thread-Id` creates the existing internal `operate` Deployment, and later internal continuation still requires its live handle. When the header names a fixed Agent Node, the route resolves that node and delegates to `AgentThreadService`, so first invocation lazily realizes its persisted external Profile and overrides while later calls reuse or recover the durable Agenetes workload. Fixed turns continue draining after the RFS socket disconnects and remain stoppable through the shared explicit stop path.
- **Deployment turns are mutually exclusive**: `AgentThreadService` owns the shared per-`threadId` turn lease for UI and fixed-node RFS invocation; the legacy internal RFS branch uses the same lease directly. The lease remains held until the run settles, including when a UI or fixed RFS client disconnects.

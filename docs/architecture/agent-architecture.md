...
- Agentlet control capability version 1 adds authenticated, host-agnostic `server/discoverHarnesses` and `server/validateNativePath` RPCs. Huabu supplies only its signed trusted harness catalogue as bounded shell-free executable probes; the target daemon reports installed, missing, or probe-failed observations and validates cwd with its native platform semantics. Connection, reconnection, and owner-requested refresh call the same machine-scoped discovery service. Older daemons are projected as unsupported and never cause Huabu to probe the Server machine instead.
- Placement identity remains the existing `agentletId`, supplied explicitly by deployment or derived by the current daemon startup behavior; hostname is separately carried as display metadata. This change does not introduce persisted UUID migration, duplicate-process enforcement, or single-instance locking. Changing an existing placement id therefore still makes workloads pinned to the old id unavailable rather than silently rebinding them.
...
- Opening Chat and opening the slash menu read only `GET /api/acp/threads/:threadId/cached-meta`. The response projects cached slash commands and selector catalogues from a live or persisted realized thread first, then from `profile-schema-cache`, and finally returns a successful empty observation; it also projects whether the durable workload is unrealized or the immutable realized machine/Profile/cwd snapshot. These reads never call `agenetes.create()`, spawn ACP, or create a WorkloadSpec. Profile-level mode/model values may be displayed as last observed; generic config-option values render without a selected value until the current thread reports them or records a successful explicit choice. Live metadata continues updating the current thread and is folded back into the Profile cache. Huabu separately remembers only successful explicit model and `thought_level` choices in the Profile's host-owned `customData`; modes, auto-approve/full-access controls, booleans, and unknown config options remain thread-only.
- Machine discovery and durable Profiles are distinct product resources. Chat may show an installed `(agentletId, harnessId)` observation directly, but explicit selection materializes or reuses one ordinary visible command Profile whose host-owned provenance and exact machine/harness/cwd match. Refresh never creates, updates, or resurrects Profiles; deletion affects new selection only, while existing workloads retain their immutable snapshot. Before realization, an unbound Chat or selectable Question may persist a target-machine cwd override; realization validates it through that agentlet and snapshots the effective override-or-Profile directory. After realization, Chat renders the durable snapshot read-only, including when the source Profile or machine is unavailable.

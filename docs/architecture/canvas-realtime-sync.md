...
Preprocessing reconciliation is selective: undo/redo retains unfinished tasks whose request inputs are unchanged and replaces tasks whose inputs changed, including Frame child labels. Interrupted deletion work resumes only after restored content successfully persists; completed nodes are not automatically reprocessed. All client task callbacks validate task ownership, Canvas, live-node presence, and the content restore barrier. This guards UI projections/status, not server-side freshness of derived-content writes; see [Node preprocessing](./node-preprocessing.md#4-triggers--state).

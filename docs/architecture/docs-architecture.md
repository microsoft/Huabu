...
The first handbook section is the task-focused `Getting Started`, and `/docs` opens its `Quick Start` article directly. The Quick Start takes a new user through installation, Home selection, Chat Model configuration, Space creation, adding one piece of material, and completing one contextual AI conversation. The `Using Huabu` section consolidates the essential Space interactions into one `Work in a Space` article instead of maintaining separate pages for every node type and canvas feature. The `Work with AI` section uses one canonical `Work with AI` article for Chat, Agent, Agent Nodes, change review, and everyday Skill use; Memory combines the user-facing background-curation explanation, while External Agents remains a separate advanced setup article. Superseded feature pages and routes are removed rather than retained as parallel explanations. Unreleased product capabilities are not registered as public handbook routes or referenced by public articles. Product positioning and showcase content stay outside this initial handbook path.

User support content lives in the `Reference` navigation group. Its `Report an Issue` article sends users to the public `microsoft/Huabu` GitHub Issues tracker and asks for reproduction steps, system information, and carefully reviewed diagnostics without creating a second feedback channel inside the handbook.

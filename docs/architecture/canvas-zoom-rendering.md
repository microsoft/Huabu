...
| [`hooks/useNodeTakeover.ts`](../../apps/web/src/hooks/useNodeTakeover.ts)                                                     | Self-subscribes to zoom + node rect; interpolates the mark's `size` and screen `point` (corner → centre) by `collapseProgress` every frame, plus the live `collapsedCenter` + `collapsedRadius` (canvas space) edges clip to. Returns `{ stage, size, point, collapsedCenter, collapsedRadius }`.                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
That last rule resolves attention from the last deliberate interaction — a pointer press or a focus change — rather than from pointer position, tracked in [`canvasAttentionStore`](../../apps/web/src/store/canvasAttentionStore.ts). Hover is the obvious signal and the wrong one: sweeping the cursor across a panel en route somewhere else would blink chrome out and back, and defending against that needs grace timers that make the chrome feel laggy in return. Press/focus is discrete and sticky — chrome disappears exactly once, when the user commits to another surface, and returns as soon as they click into the canvas again. Selection itself is never cleared by this, so returning resumes where they left off. Canvas ownership is expressed in the DOM: the React Flow wrapper carries `data-canvas-root`, and floating chrome — which portals to `document.body` and is therefore outside that subtree — opts in with `data-canvas-chrome` so operating a toolbar does not read as leaving the canvas.
...
Route every new floating canvas surface through `CanvasFloatingPopover` rather than portalling it directly, so it inherits the surface-level suppression rules instead of growing a parallel set. Add a new app-wide suppression reason there (or, for modals, to `useAnyGlobalModalOpen`); keep reasons that depend on which node is involved in the `NodeWrapper` gate.

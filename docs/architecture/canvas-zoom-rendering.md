...
Structured Frame drop feedback uses one visual grammar for `column`, `row`, and `grid`: quiet borderless bands show the active track, while the precise insertion caret or new-track ghost remains the primary signal; `grid` adds a second quiet band for the shared row and uses the solver's final cell footprint instead of a masonry caret. Internal nodes are not outlined because combining peer boxes with row and column bounds creates competing geometry. [`StructuredDropOverlay`](../../apps/web/src/components/Panels/Canvas/StructuredDropOverlay.tsx) projects the shared helper's canvas-space geometry into the screen-space HUD and never re-derives layout from DOM geometry. The helper delegates its decision to the same picker and solver used at commit, so preview and final placement cannot disagree.
...
A node collapsed to its takeover mark is the one case where chrome must not use the node's own rect. The footprint survives collapse — it stays selectable and keeps receiving pointer events — but it has faded to zero opacity, so a selection outline, floating toolbar, or port drawn against it frames empty canvas beside a mark it appears unrelated to. Everything therefore anchors to the mark's canvas-space bounding square, published per node by [`nodeCollapseStore`](../../apps/web/src/store/nodeCollapseStore.ts) and read through `collapsedMarkRect`: the selection outline becomes a circle around the mark, the floating toolbar anchors above it, and the four connect ports move to the square's edge midpoints — which are also the mark circle's cardinal points, so one rectangular formula serves both the readable card and the collapsed mark. The outline stays circular rather than following the rectangular convention the readable card uses, because `collapsedRadius` makes that square exactly as wide as the mark's disc: a circle hugs it with no gap, whereas any squarer corner exposes bare canvas that reads as a plate behind the mark. Resize handles are dropped entirely while collapsed — there is no visible card whose size they would describe.
...
| [`apps/web/src/components/Panels/Canvas/StructuredDropOverlay.tsx`](../../apps/web/src/components/Panels/Canvas/StructuredDropOverlay.tsx)             | Structured Frame track, alignment-peer, and insertion feedback              |
...
| [`apps/web/src/store/nodeCollapseStore.ts`](../../apps/web/src/store/nodeCollapseStore.ts)                                                             | Collapsed-mark geometry edges and chrome anchor to                          |

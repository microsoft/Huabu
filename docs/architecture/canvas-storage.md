...
**A process serves one Workspace** (issue #126). The Workspace is named by `HUABU_WORKSPACE` (the operator's, locked, and a failure to open it fails the boot), by `HUABU_WORKSPACE_STARTUP` (the shell's record of the user's own choice, which stays free mode and reports a failure through `WorkspaceInfo.startupError`), or by the single runtime activation a still-unconfigured process accepts. Asking for a Workspace other than the active one returns `WORKSPACE_RESTART_REQUIRED` without mutating anything — the choice is valid, and applying it is the next process's job.

Runtime Home-folder activation prepares and migrates the selected directory in a disposable child process before committing it as the active Workspace. This isolation is required because synchronous filesystem calls against cloud, network, or virtual drives can block indefinitely; a stuck preparation is terminated after 70 seconds with `WORKSPACE_ACTIVATION_TIMEOUT` and commits nothing, leaving the process unconfigured rather than half-activated — the state the client already recovers from by showing the picker. Concurrent activation attempts return `WORKSPACE_ACTIVATION_IN_PROGRESS`. Both env-named Workspaces are prepared synchronously at startup, before the Server accepts requests.
...
| `backends/disk/legacy/`                         | The legacy `CanvasStore` and its synchronous adapter primitives, its bounded instance cache, and process-local node tombstones.                                     |
...
No Disk handle, repository, blob scope, instance cache, or node fence records _which_ Workspace it belongs to, because a process only ever serves one. Committing a Workspace drops all of them together (`resetStorage()`), so the next handle is new — which is what keeps a test moving between temporary Workspaces from reading a stale one. Each `spaces()` call returns a fresh handle and each read rescans current Disk state. The delete-session contract covers overlapping operations through one configured backend instance. Disk realizes it with the shared process-local coordinator; it is not a multi-process transaction or distributed lock, and a SQL adapter must supply an equivalent backend-instance fence using its own mechanisms.

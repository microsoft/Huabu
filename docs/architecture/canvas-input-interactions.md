...
Mouse Pan also has a Canvas-bound release guard for temporary Pan, persistent Pan, and middle-button Pan. Native `mouseup` remains authoritative; if a matching `pointerup` is observed but no `mouseup` arrives before the next animation frame, a later pointer move reports no pressed buttons, the pointer is cancelled, or the window loses focus, the guard dispatches a window-level fallback `mouseup` so React Flow's d3-zoom session removes its global move listeners instead of leaving the viewport attached to later mouse movement.

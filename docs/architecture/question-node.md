...
| Server lifecycle     | [agent-node-lifecycle.ts](../../apps/server/src/modules/agent/agent-node-lifecycle.ts) provides serialized first-content, running, done, and error patches through the canonical Canvas executor; it becomes authoritative when fixed-node invocation is routed through the server service                                                                    |

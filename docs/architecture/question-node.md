...
The owner node persists the initial inherited selection and subsequent Editing draft changes through acknowledged Canvas commands, including mode and an optional external-agent working-directory override. `resolveConversationAgentBinding` and `resolveConversationLaunchOverrides` are shared by panel and send and read the owner before the ChatStore cache. The Chat working-directory control validates typed paths on the selected execution machine and stays editable only before canonical binding. Saves require an applied command and version-aware reconciliation; a failed or conflicting save prevents dispatch, and a delayed response cannot replay an optimistic patch over newer SSE state. A realized external workload is authoritative even when realization happened through a control before any message, and its machine/Profile/cwd projection is read-only. Built-in model/reasoning settings remain cached until a durable thread owns them. The browser owns unsent input and request feedback, not node lifecycle or history-based repair.

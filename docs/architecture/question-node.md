...
As the canvas zooms out, a question node with agent status opts into `minimal` semantic-zoom LOD and collapses to its agent avatar as a centred stand-in ([QuestionMinimalAvatar](../../apps/web/src/components/Nodes/question/QuestionMinimalAvatar.tsx)): the card cross-fades out, the corner badge fades with it, and the avatar rides [`avatarSizeForNode`](../../apps/web/src/config/agentAvatarLOD.ts) down toward a solid identity dot ([AgentAvatarMark](../../apps/web/src/components/Common/AgentAvatarMark.tsx)) so a field of zoomed-out question nodes reads as tidy colour-coded dots. Status colour is shared with the corner badge via [questionBadgeChrome.ts](../../apps/web/src/components/Nodes/question/questionBadgeChrome.ts); an idle question node (no status) falls back to the generic title label. See [canvas-zoom-rendering.md](./canvas-zoom-rendering.md#3-node-level-of-detail) and [proposals/question-node-zoom-lod-avatar.md](../proposals/question-node-zoom-lod-avatar.md).
...
| Agent status badge  | [QuestionAgentBadge.tsx](../../apps/web/src/components/Nodes/question/QuestionAgentBadge.tsx)                                                      |

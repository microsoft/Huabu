...
| RFS delegation       | [rfs.route.ts](../../apps/server/src/modules/remote_fs/rfs.route.ts) exposes `POST /agent/create` over `AgentNodeService` and routes fixed-thread `POST /agent` calls through `AgentThreadService`; the legacy internal live-thread branch remains compatible                                                                                                    |

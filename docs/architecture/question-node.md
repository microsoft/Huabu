...
As the canvas zooms out, a question node's agent mark **takes over** as the node's stand-in through three discrete stages ([QuestionTakeoverMark](../../apps/web/src/components/Nodes/question/QuestionTakeoverMark.tsx)): `readable` (the sticky card plus a corner badge that scales with the card and always shows the full character), `avatar` (once the text is no longer legible the card fades out and a centred agent avatar fills the node footprint), and `dot` (below ~22 px it collapses to a solid identity dot, via [AgentAvatarMark](../../apps/web/src/components/Common/AgentAvatarMark.tsx)), so a field of zoomed-out question nodes reads as tidy colour-coded dots. An idle (never-asked) node shows a quiet neutral dot instead of borrowing an agent's identity colour. Each resting stage is crisp; only the transition between stages animates (a FLIP slide + resize). The staging is driven by the takeover engine ([useNodeTakeover](../../apps/web/src/hooks/useNodeTakeover.ts) / [NodeTakeoverLayer](../../apps/web/src/components/Nodes/NodeTakeoverLayer.tsx)); status colour is shared via [questionBadgeChrome.ts](../../apps/web/src/components/Nodes/question/questionBadgeChrome.ts). See [canvas-zoom-rendering.md#31-three-stage-takeover-question-node](./canvas-zoom-rendering.md#31-three-stage-takeover-question-node) and [proposals/question-node-zoom-lod-avatar.md](../proposals/question-node-zoom-lod-avatar.md).

...
`NodeRefNode` keeps persistent target identity separate from a runtime `worldReferences` map populated by the batch reference endpoint. The resolved source projection includes label/type/summary/preview/revision and, for question nodes, thread/lifecycle/mode/binding fields. It renders source details or an explicit missing-Space/missing-node placeholder and refreshes on World load, shortcut open, window focus, headless turn start/end, and active-World Pin/Unpin completion. Source-Space single/multi-selection toolbars expose explicit Pin and Unpin actions, while World selections expose Unpin for selected `nodeRef` entries. These actions drain pending, in-flight, and coalesced structure writes before calling the server command boundary and never construct positions or references in web state.

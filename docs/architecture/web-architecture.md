...
Which node the panel shows is owned by the preview workspace, not by `canvasStore`: the panel renders the focused group's active tab when that tab targets a node. `openExpanded` and `closeExpanded` remain the entry points and delegate to the workspace one way, so the panel and the workspace cannot disagree. The panel always sits beside the Canvas — the earlier mode that let it replace the Canvas entirely is gone, because the workspace's maximum width always preserves a minimum Canvas width. See [`docs/proposals/unified-preview-workspace.md`](../proposals/unified-preview-workspace.md).

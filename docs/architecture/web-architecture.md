...
`NodeRefNode` keeps persistent target identity separate from a runtime `worldReferences` map populated by the batch reference endpoint. It renders resolved source label/type/summary or an explicit missing-Space/missing-node placeholder, and refreshes on World load, window focus, and active-World Pin/Unpin completion. Source-Space single/multi-selection toolbars expose explicit Pin and Unpin actions, while World selections expose Unpin for selected `nodeRef` entries. These actions drain pending, in-flight, and coalesced structure writes before calling the server command boundary and never construct positions or references in web state.

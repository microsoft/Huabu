...
Which node the panel shows is owned by the preview workspace, not by `canvasStore`: the panel renders the focused group's active tab when that tab targets a node. `openExpanded` and `closeExpanded` remain the entry points and delegate to the workspace one way, so the panel and the workspace cannot disagree. With the workspace feature enabled, `MainLayout` mounts the workspace in its collapsible right column and `CenterArea` hosts only the Canvas; opening an empty right workspace lazily creates one unbound Chat, and closing its final tab collapses the column. Tabs use shared pointer and keyboard drag sensors, but every drop delegates to the workspace model's `moveTab` action for same-group ordering, cross-group movement, active-tab repair, and empty-group removal. The flag-off rollback path retains the legacy right `ChatPanel` and side-by-side Expanded Node rendering. The earlier mode that let a preview replace the Canvas entirely is gone. See [`docs/proposals/unified-preview-workspace.md`](../proposals/unified-preview-workspace.md).

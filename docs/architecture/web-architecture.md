...
The Expanded Node Panel derives connected-node navigation from the active Canvas edges without adding persisted navigation state. One relationship-menu trigger sits at the far left before the node title and groups destinations as sources, neighbors, and destinations instead of exposing three persistent toolbar buttons. Node-specific preview actions sit on the right before a divider and the view controls. A `forward` arrow follows the edge's source-to-target endpoints, a `backward` arrow reverses them, and a `both` arrow contributes the neighbor to both source and destination groups; a `none` edge has no directional meaning and appears in the neighbor group. Neighbors follow Canvas node order after missing endpoints, self-loops, and duplicates are removed. Bare Left/Right Arrow actions switch directly when one directional neighbor exists or open the relationship menu focused on the matching group when several exist; neutral connections remain menu-driven so the directional shortcuts do not imply an invented order. Editable controls, search, menus, media controls, and embedded viewers retain arrow-key ownership. Switching reuses `openExpanded` and does not select or reveal the destination on the Canvas. It opens transiently: navigating between connected nodes is browsing, so it reuses the preview group's inspection slot rather than accumulating a tab per neighbor.

Which node the panel shows is owned by the preview workspace, not by `canvasStore`: the panel renders the focused group's active tab when that tab targets a node. `openExpanded` and `closeExpanded` remain the entry points and delegate to the workspace one way, so the panel and the workspace cannot disagree. In workspace mode, opening a node also expands the host right column when needed; a Canvas-background click closes the focused node tab without collapsing the workspace, allowing another tab or the lazily seeded Chat to take its place. With the workspace feature enabled, `MainLayout` mounts the workspace in its collapsible right column and `CenterArea` hosts only the Canvas; opening an empty right workspace lazily creates one unbound Chat, and closing its final tab collapses the column. The workspace tab strip and embedded Chat or Expanded Node action bars share a 36px height. Embedded action bars are borderless because the tab strip already separates navigation from content, and the tab close control is the only close affordance. The tab strip owns the active target's primary title; an embedded Expanded Node action bar repeats the name only as a small, subtle, breadcrumb-like rename affordance and expands the existing inline editor when activated. An unbound Chat suppresses the duplicate generic title while retaining save controls, and a Question conversation retains its renameable title without the legacy back button. New-conversation creation is a workspace-level tab-strip action available from every target; it creates and activates an independent Chat tab in that group while preserving the source tab and its thread history. The flag-off single-panel path retains its header menu and still replaces its global current thread. Tool groups render a divider only when another control follows it, so compact Chat and Expanded Node action bars have no trailing separator. Every tab retains its right divider, including the final tab, while the strip keeps one continuous subtle bottom divider and the active tab uses the surface background plus a top info-light accent. Tabs size to their content by default, flex down to their minimum width when crowded, and only then make the strip scroll horizontally. Tabs use shared pointer and keyboard drag sensors, but every drop delegates to the workspace model's `moveTab` action for same-group ordering, cross-group movement, active-tab repair, and empty-group removal. The flag-off rollback path retains the legacy right `ChatPanel` and side-by-side Expanded Node rendering with its full header. The earlier mode that let a preview replace the Canvas entirely is gone. See [`docs/proposals/unified-preview-workspace.md`](../proposals/unified-preview-workspace.md).

...
6. Derive one concise, single-line issue title no longer than 80 characters. Create one durable Task with the full investigation goal, then use the returned `anchorNodeId` with `POST /execute` and `MERGE_NODE_DATA` to set the Task Note label to that title. The title is display metadata only; never shorten the Task goal.
7. Read `fixing-agent-preamble.md` from the managed workspace. In standalone Skill use, read `references/fixing-agent-preamble.md` relative to the Skill package. Create one Run with the new worktree as `workingDirPath` and with `additionalInitialPreamble` equal to the complete preamble file followed by `Expected worktree path: <absolute path>` and `Expected branch: <branch name>` on separate lines. The full Task goal must tell the root Agent to read repository instructions and relevant architecture documentation, investigate and reproduce the issue, identify the root cause, and propose a focused solution without editing files.
8. Use the returned `rootNodeId` with `POST /execute` and `MERGE_NODE_DATA` to set the root Agent Node label to `<concise issue title> · Investigation Agent`. Save the Task, Run, root Node, and root Thread identities. Never create another Run or Agent for this issue unless the user explicitly requests a restart.

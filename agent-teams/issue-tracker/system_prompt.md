...
7. Create one Run in the new worktree. Tell the root Agent to read repository instructions and relevant architecture documentation, investigate and reproduce the issue, identify the root cause, and propose a focused solution without editing files. Use the returned `rootNodeId` with `POST /execute` and `MERGE_NODE_DATA` to set the root Agent Node label to `<concise issue title> · Investigation Agent`.
8. Save the Task, Run, root Node, and root Thread identities. Never create another Run or Agent for this issue unless the user explicitly requests a restart.

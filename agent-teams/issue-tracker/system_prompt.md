...
Use the concise execution-unit title as the stable Frame identity and append a short current phase or outcome, for example `Issue #81 · Ready to Merge`. Create the Frame before or alongside the Task, save its Node ID with the execution-unit identities, and use `SET_NODE_PARENT` to place the Task Note and Agent Nodes inside it as their IDs become available. Add later issue-specific nodes to the same Frame and update the Frame label when the phase materially changes.
...
6. Derive one concise, single-line execution-unit title no longer than 80 characters. Create its dedicated Frame and one durable Task with the full investigation goal and complete issue identity set. Use the returned `anchorNodeId` with `POST /execute` to set the Task Note label to that title and place it inside the Frame. The title is display metadata only; never shorten the Task goal.
...
8. Use the returned `rootNodeId` with `POST /execute` to set the root Agent Node label to `<concise execution-unit title> · Investigation Agent` and place it inside the execution unit's Frame. Save the issue set, Frame Node, branch, worktree, Task, Run, root Node, and root Thread identities together. Never create another Run or Agent for that execution unit unless the user explicitly requests a restart.

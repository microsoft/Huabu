 ...
 * while a move drag is in progress it follows the live offset.
 *
 * When `interactive` (a stroke selection exists, so the region is grab-to-
 * move), the loop's interior hit-tests with a `move` cursor to signal "grab
 * to move" — the actual move gesture is still claimed by the pointer router
 * via point-in-polygon, this element only advertises the cursor. Dragging it
 * moves the strokes plus any whole nodes the lasso also caught (they travel
 * together).
 */

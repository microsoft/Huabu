...
type ShowcaseScene = {
  ...
  body: string;
  /**
   * Placeholder image path. Drop a real PNG into
   * `apps/web/public/docs/showcase/` and the asset will be served
   * at `/docs/showcase/<file>.png`.
   */
  image?: string;
  /** Optional jump-to link rendered as a subtle CTA. */
  cta?: { label: string; to: string };
  /** When true, render the image on the left and copy on the right. */
  flip?: boolean;
};
...
const scenes: ShowcaseScene[] = [
  {
    ...
    image: '/docs/showcase/research.png',
  ...
  },
  {
    ...
    image: '/docs/showcase/brainstorm.png',
  },
  {
...
function Scene({ scene }: { scene: ShowcaseScene }) {
  return (
    <section
      className={cn(
        'grid items-center gap-x-12 gap-y-6 lg:grid-cols-2',
        scene.flip && 'lg:[&>*:first-child]:order-2',
      )}
    >
      <ImagePanel src={scene.image} alt={scene.title} />
...
function ImagePanel({ src, alt }: { src?: string; alt: string }) {
  return (
    <div className="aspect-[16/10] overflow-hidden rounded-2xl border border-gray-200 bg-gradient-to-br from-gray-50 to-gray-100">
      {src ? (
        <img
          src={src}
          alt={alt}
          className="h-full w-full object-cover"
          // Graceful fallback — show the placeholder gradient if the
          // image hasn't been added yet so the page never breaks.
          onError={(event) => {

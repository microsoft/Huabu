...
export async function ingestNodeIfNeeded({
  ...
  try {
    ...
    // Only send title if it was explicitly set by the user
    ...
    if (response.success && response.suggestedLabel) {
      ...
      if (suggestedLabel.length > 0) {
        ...
        if (currentNode) {
          ...
          // Check if current label was auto-generated (not user-set)
          const labelSource = (currentNode.data as Record<string, unknown>)
            ?.labelSource as string | undefined;
          const isAutoLabel = !labelSource || labelSource === 'auto';

          if (isEmptyLabel || isAutoLabel) {

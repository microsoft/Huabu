...
export async function ingestNodeIfNeeded({
  ...
  try {
    ...
    const isAutoGeneratedPlaceholder =
      /^(Frame|Image|Video|Web|PDF|Note|Text|Untitled) \d+$/.test(currentLabel);
    ...
    if (response.success && response.suggestedLabel) {
      ...
      if (suggestedLabel.length > 0) {
        ...
        if (currentNode) {
          ...
          // Patterns: "Frame 1", "Image 2", "Web 3", "PDF 1", etc.
          const isAutoGeneratedPlaceholder =
            /^(Frame|Image|Video|Web|PDF|Note|Text|Untitled) \d+$/.test(
              currentLabel,
            );

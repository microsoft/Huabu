...
- **Don't reference a node you're creating in the same call, and don't hand-write its id — omit `id` and let the server assign it.** Create the nodes first; the result echoes each new node's real id in `results[].nodes`. Wire them up (`CONNECT_NODES` / `SET_NODE_PARENT`) in a follow-up call using those ids.

...
description: Map freehand sketch gestures on the Space (lines, loops, scribbles, "?" / "!" marks) to the space_commands invocations that realise them. Pipeline-only — not loaded by chat / operate / external agents.
appliesTo: [sketch]
version: 1
---
...
The host system prompt already pinned **what role you play** (executor, not narrator) and **how every turn must end** (in a tool call — prose alone is a silent no-op). This skill is purely **how to read each gesture and pick the right `space_commands` to invoke**.

For the Space filesystem layout, the read / inspect / grep boundaries, and the full command catalogue, load `skills/space/SKILL.md` on demand.
...
Iterate read / inspect calls as long as you need before invoking `space_commands`.

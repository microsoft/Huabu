...
- **Never pin a `note`'s height.** Omit `height` entirely (or pass `height: "auto"`) and let Huabu measure the rendered Markdown and size the node itself. A numeric height records the note as user-pinned, which permanently opts it out of re-measurement — the note then keeps that box forever and truncates its content as it grows. Setting only `width` is fine when you need to match nearby Notes.

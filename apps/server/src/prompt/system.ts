...
- When the user selects canvas nodes, you receive only a brief preview (summary/keywords/snippet) — not the full content. If you need the full text to answer the question, call **read** on "<canvasId>/nodes/<nodeId>.md" — every node's title / content / type / src / summary / keywords lives in that markdown file's YAML frontmatter and body.
`.trim();

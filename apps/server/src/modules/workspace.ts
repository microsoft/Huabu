 ...
 * storage connection, cache, and directory index be opened once and trusted
...
import { resetStorage } from './storage/storage.js';
...
/**
 * Drop everything opened against whichever workspace was active.
 *
 * A mount, a directory index, a skill cache, and the external-note watchers
 * are all workspace-scoped. The import cycle with the prompt loader (which
 * depends on `getWorkspacePath` from this module) is safe because Node ESM
 * allows cycles as long as no top-level code on either side dereferences the
 * late-bound import — `invalidateUserSkill` is only called from inside a
 * function body, after both modules have finished evaluating.
 */

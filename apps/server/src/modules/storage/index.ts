 ...
 * access). Each entry is paired with its persisted `space.json`; rows
 ...
 * `scanWorkspace()` parsed each `space.json`. That avoids re-reading
 ...
 * mutate disk. The persisted `space.json` is reconciled lazily the next
...
/**
 * Create an empty Space folder + `space.json`. The directory is

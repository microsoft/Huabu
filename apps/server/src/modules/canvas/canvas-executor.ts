 ...
 * authoritative `<canvasDir>/space.json` state, persists both the
...
export async function executeOnServer(
  ...
  // fresh artifact files (unique ids, no space.json contention), so keeping it
  ...
  return await withCanvasMutex(canvasId, async () => {
    ...
    // `data.content` to merge against, but space.json never carries it.
    ...
    // leaving the agent with `applied: true` while space.json on disk
    // is unchanged.
    //
    // We do NOT synthesise a delta — Phase A has no order-aware delta
    // type, and cross-tab broadcast (M3) is not shipped yet. We just
    // fall through to the persistence branch so space.json and the
    ...
    // Persist .md sidecars FIRST so space.json never references a
    // markdown file that does not exist on disk. A crash between this
    // loop and the space.json write leaves orphan .md files (harmless),
    ...
    // batch aborts before space.json is mutated. The exception bubbles
 ...
 * persist `.md` + space.json → append delta-log → bump version) but

 ...
 * This module is just an adapter: it loads `space.json`, normalizes
 ...
 *   - This module owns whatever lives in `space.json`: position, size,
 ...
 * Space coordinates. `space.json` stores positions relative to the
...
export function buildCanvasOutline(
  ...
  // Resolve display labels from the sidecar (space.json never carries
...
export function inspectNodes(
  ...
  // Display labels come from the sidecar (space.json never carries them);

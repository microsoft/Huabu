...
export default function ChatPanel() {
  return (
    <PageLayout
      title="Chat Panel"
      description="The chat panel is where you talk to the AI without leaving the Space. It docks on the right, stays out of the way until you need it, and always sees what's on the Space — there's no separate 'context window' to manage."
      toc={toc}
    >
      <H2>Opening the panel</H2>
      <P>
        Toggle the panel from the floating button in the top-right of the Space
        (or press <Kbd>Ctrl</Kbd>/<Kbd>Cmd</Kbd>+<Kbd>K</Kbd>). It slides in
        over the right edge of the Space and pushes nothing — the Space stays
        ...
        rows={[
          ...
          [
            ...
            <>
              ...
              <Code>@</Code> picker for referencing Space nodes.
            </>,
          ],
        ]}
      />
      ...
      <P>
        ...
        newline. Selected nodes on the Space are sent with the message
        automatically — there&apos;s nothing to attach by hand. To send without
        the current selection, click empty Space first to clear it.
      </P>
      <Callout tone="tip">
      ...
      <H2>Threads</H2>
      <P>
        Each Space has its own set of threads. The thread switcher in the header
        lists them by title; the New-thread button starts a fresh one with empty
        history. Switching threads is non-destructive — every thread is
        persisted under the Space folder&apos;s <Code>.history/</Code>{' '}
        directory.
      </P>
      ...
      <ul className="list-disc space-y-1.5 pl-5 text-[15px] leading-relaxed text-gray-700">
        ...
        <li>
          <strong>Operate</strong> — structured Space edits. Replies arrive as a
          previewable change list on the Space (see below).
        </li>
        <li>
      ...
      <ul className="list-disc space-y-1.5 pl-5 text-[15px] leading-relaxed text-gray-700">
        <li>
          ...
          — type <Code>@</Code> in the composer to fuzzy-search Space nodes by
      ...
      <P>
        When Operate mode produces Space edits, a change-list panel slides in
        next to the Space. Each operation gets its own card with a diff and
